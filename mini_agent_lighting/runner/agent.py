...
from mini_agent_lighting.reward import emit_reward, find_final_reward
from mini_agent_lighting.store.base import LightningStore
from mini_agent_lighting.tracer.base import Tracer
from mini_agent_lighting.tracer.otel import OtelTracer
...
from mini_agent_lighting.utils.system_snapshot import system_snapshot

if TYPE_CHECKING:
    from mini_agent_lighting.execution.events import ExecutionEvent
...
class LitAgentRunner(Runner[T_task]):
        ...
        **kwargs: Any,
        ...
        for hook in self._hooks:
            try:
                ...
                logger.exception(
                    f"{self._log_prefix()} Exception during {hook_type} hook {hook}."
                )
    ...
    async def _post_process_rollout_result(
        ...
        if isinstance(raw_result, (bool, int, float)):
            ...
            sequence_id = await store.get_next_span_sequence_id(
        ...
        if isinstance(raw_result, list):
            ...
            if len(raw_result) > 0 and all(
                isinstance(t, ReadableSpan) for t in raw_result
            ):
            ...
            # Case 4: result is a list of SpanCoreFields (agentlightning spans)
            elif len(raw_result) > 0 and all(
                isinstance(t, SpanCoreFields) for t in raw_result
            ):
                ...
                trace_spans = [
                    ...
                    for span_core_fields, sequence_id in zip(
                        raw_result, sequence_ids, strict=True
                    )
                ]
    ...
    async def _emit_heartbeat(self, store: LightningStore) -> None:
        ...
        try:
            ...
            logger.exception(
                "%s Unable to acquire heartbeat snapshot.", self._log_prefix()
            )
            return

        try:
            await asyncio.wait_for(
                store.update_worker(worker_id, snapshot),
                timeout=self._heartbeat_interval,
            )
            ...
            logger.exception(
                "%s Unable to update worker heartbeat.", self._log_prefix()
            )
    ...
    def _start_heartbeat_loop(
        ...
        if self.worker_id is None:
            logger.warning(
                "%s Cannot start heartbeat loop without worker_id.", self._log_prefix()
            )
            return None
        ...
        raise ValueError(
            f"Unsupported heartbeat launch mode: {self._heartbeat_launch_mode}"
        )

    def _start_heartbeat_asyncio_loop(
        self, store: LightningStore
    ) -> Optional[Callable[[], Awaitable[None]]]:
        ...
        task = asyncio.create_task(
            heartbeat_loop(), name=f"{self.get_worker_id()}-heartbeat"
        )
    ...
    def _start_heartbeat_thread_loop(
        self, store: LightningStore
    ) -> Optional[Callable[[], Awaitable[None]]]:
        ...
        def producer() -> None:
            ...
            while not stop_evt.is_set():
                try:
                    logger.debug(
                        f"{self._log_prefix()} Heartbeat producer: acquiring snapshot."
                    )
                    snap = system_snapshot(self._heartbeat_include_gpu)  # sync
                    logger.debug(
                        f"{self._log_prefix()} Heartbeat producer: snapshot acquired."
                    )
                    ts = time.monotonic()
                    with lock:
                    ...
                    logger.warning(
                        "%s Heartbeat producer: system_snapshot failed.",
                        self._log_prefix(),
                        exc_info=True,
                    )
            ...
            try:
                while not stop_evt.is_set():
                    ...
                    if snap is None:
                        ...
                        logger.debug(
                            "%s Heartbeat consumer: no snapshot yet; skipping update.",
                            self._log_prefix(),
                        )
                    ...
                    try:
                        logger.debug(
                            f"{self._log_prefix()} Heartbeat consumer: updating worker."
                        )
                        ...
                        logger.debug(
                            f"{self._log_prefix()} Heartbeat consumer: worker updated."
                        )
                    except asyncio.TimeoutError:
                        logger.warning(
                        ...
                        logger.warning(
                            "%s Heartbeat consumer: update failed.",
                            self._log_prefix(),
                            exc_info=True,
                        )
        ...
        t_prod = threading.Thread(
            target=producer, name=f"{worker_id}-heartbeat-producer", daemon=True
        )
        t_cons = threading.Thread(
            target=consumer, name=f"{worker_id}-heartbeat-consumer", daemon=True
        )
        t_prod.start()
        t_cons.start()
    ...
    async def _sleep_until_next_poll(
        ...
        """
        interval = self._poll_interval + self._random_state.uniform(
            -self._interval_jitter, self._interval_jitter
        )
        interval = max(interval, 0.01)
        if event is None:
    ...
    async def _step_impl(
        self, next_rollout: AttemptedRollout, raise_on_exception: bool = False
    ) -> str:
        ...
        if resources_id:
            ...
            logger.debug(
                f"{self._log_prefix(rollout_id)} No 'resources_id'. Fetching latest resources."
            )
            resources_update = await store.get_latest_resources()
        if not resources_update:
            if raise_on_exception:
                raise RuntimeError(
                    f"{self._log_prefix(rollout_id)} Failed to fetch resources"
                )
            else:
                logger.error(
                    f"{self._log_prefix(rollout_id)} Failed to fetch resources. Skipping."
                )
                return rollout_id

        logger.debug(
            f"{self._log_prefix(rollout_id)} Resources fetched (id={resources_update.resources_id})."
        )
        ...
        try:
            ...
            async with self._tracer.trace_context(
                ...
                attempt_id=next_rollout.attempt.attempt_id,
            ):
                logger.debug(f"{self._log_prefix(rollout_id)} Entered trace context.")
                await self._trigger_hooks(
                    hook_type="on_trace_start",
                    agent=agent,
                    runner=self,
                    tracer=self._tracer,
                    rollout=next_rollout,
                ...
                if agent.is_async():
                    rollout_method = (
                        agent.training_rollout_async
                        if next_rollout.mode == "train"
                        else agent.validation_rollout_async
                    )
                    logger.debug(
                        f"{self._log_prefix(rollout_id)} Starting async rollout method."
                    )
                    ...
                    logger.debug(
                        f"{self._log_prefix(rollout_id)} Async rollout method completed."
                    )
                else:
                    rollout_method = (
                        agent.training_rollout
                        if next_rollout.mode == "train"
                        else agent.validation_rollout
                    )
                    logger.debug(
                    ...
                    logger.debug(
                        f"{self._log_prefix(rollout_id)} Sync rollout method completed."
                    )

                await self._trigger_hooks(
                    hook_type="on_trace_end",
                    agent=agent,
                    runner=self,
                    tracer=self._tracer,
                    rollout=next_rollout,
            ...
            logger.exception(
                f"{self._log_prefix(rollout_id)} Exception during rollout."
            )
            has_exception = True
            ...
            try:
                ...
                logger.exception(
                    f"{self._log_prefix(rollout_id)} Exception during on_rollout_end hook."
                )
            ...
            try:
                if has_exception:
                    ...
                    await store.update_attempt(
                        rollout_id, next_rollout.attempt.attempt_id, status="failed"
                    )
                else:
                    await store.update_attempt(
                        rollout_id, next_rollout.attempt.attempt_id, status="succeeded"
                    )
            except Exception:
                logger.exception(
    ...
    async def iter(self, *, event: Optional[ExecutionEvent] = None) -> None:
        ...
        logger.info(
            f"{self._log_prefix()} Started async rollouts (max: {self._max_rollouts or 'unlimited'})."
        )
        store = self.get_store()
        ...
        try:
            while not (event is not None and event.is_set()) and (
                ...
                while not (event is not None and event.is_set()):
                    ...
                    logger.debug(
                        f"{self._log_prefix()} Next rollout retrieved: {next_rollout}"
                    )
        ...
        logger.info(
            f"{self._log_prefix()} Finished async rollouts. Processed {num_tasks_processed} tasks."
        )
    ...
    async def step(
        ...
        *,
        ...
        if completed_rollout is None:
            raise RuntimeError(
                f"{self._log_prefix()} Failed to fetch completed rollout by id after step: {rollout_id}"
            )
