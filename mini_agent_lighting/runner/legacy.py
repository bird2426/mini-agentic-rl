...
from mini_agent_lighting.adapter import TracerTraceToTriplet
from mini_agent_lighting.client import AgentLightningClient
from mini_agent_lighting.litagent import LitAgent
from mini_agent_lighting.litagent.litagent import is_v0_1_rollout_api
...
class LegacyAgentRunner(Runner[Any]):
    ...
    def _to_rollout_object(
        ...
        if isinstance(result, list) and all(
            isinstance(t, (ReadableSpan)) for t in result
        ):
            trace_spans = result  # type: ignore
            trace = [
                json.loads(readable_span.to_json()) for readable_span in trace_spans
            ]  # type: ignore
    ...
    def run(self) -> bool:  # type: ignore
        ...
        rollout_obj = RolloutLegacy(
            rollout_id=task.rollout_id, task=task
        )  # Default empty rollout
        ...
        try:
            try:
                ...
                logger.exception(
                    f"{self._log_prefix(rollout_id)} Exception during on_rollout_start hook."
                )
                ...
                if is_v0_1_rollout_api(rollout_method):
                    result = cast(
                        ...
                        rollout_method(
                            ...
                            resources=resources_update.resources,  # type: ignore
                        ),
                    )  # type: ignore
                else:
    ...
    def iter(self) -> int:  # type: ignore
        ...
        logger.info(
            f"{self._log_prefix()} Started sync rollouts (max: {self.max_tasks or 'unlimited'})."
        )
        ...
        while self.max_tasks is None or num_tasks_processed < self.max_tasks:
            ...
            if num_tasks_processed % 10 == 0 or num_tasks_processed == 1:
                logger.info(
                    f"{self._log_prefix()} Progress: {num_tasks_processed}/{self.max_tasks or 'unlimited'}"
                )

        logger.info(
            f"{self._log_prefix()} Finished sync rollouts. Processed {num_tasks_processed} tasks."
        )
    ...
    async def iter_async(self) -> int:
        ...
        num_tasks_processed = 0
        logger.info(
            f"{self._log_prefix()} Started async rollouts (max: {self.max_tasks or 'unlimited'})."
        )
