...
from mini_agent_lighting.semconv import AGL_VIRTUAL
...
def extract_extra_fields(src: Any, excluded_fields: List[str]) -> Dict[str, Any]:
    ...
    candidates = {
        ...
        if k not in excluded_fields_set and not k.startswith("_")
    }
...
class SpanContext(BaseModel):
    ...
    def from_opentelemetry(cls, src: trace_api.SpanContext) -> "SpanContext":
        ...
        return cls(
            ...
            trace_state={k: v for k, v in src.trace_state.items()}
            if src.trace_state
            else {},
            **extract_extra_fields(
                src, ["trace_id", "span_id", "is_remote", "trace_state"]
            ),
        )
...
class Span(BaseModel):
    ...
    def from_opentelemetry(
        ...
        return cls(
            ...
            parent_id=(
                trace_api.format_span_id(src.parent.span_id) if src.parent else None
            ),
            name=src.name,
            status=TraceStatus.from_opentelemetry(src.status),
            attributes=dict(src.attributes) if src.attributes else {},
            events=[Event.from_opentelemetry(event) for event in src.events]
            if src.events
            else [],
            links=[Link.from_opentelemetry(link) for link in src.links]
            if src.links
            else [],
    ...
    def from_attributes(
        ...
        *,
        ...
        trace_id = trace_id or trace_api.format_trace_id(
