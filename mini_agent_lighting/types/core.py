...
if TYPE_CHECKING:
    ...
    from mini_agent_lighting.runner.base import Runner
...
RolloutRawResultLegacy = Union[
    None, float, List[Triplet], List[Dict[str, Any]], List[ReadableSpan], RolloutLegacy
]
...
class Hook(ParallelWorkerBase):
    ...
    async def on_rollout_start(
        self, *, agent: LitAgent[Any], runner: Runner[Any], rollout: Rollout
    ) -> None:
...
class PaginatedResult(BaseModel, Sequence[T_item]):
    ...
    def __repr__(self) -> str:
        ...
        items_repr = (
            f"[{first_item_repr}, ...]" if len(self.items) > 1 else first_item_repr
        )
        slice_repr = (
            f"{self.offset}:"
            if self.limit == -1
            else f"{self.offset}:{self.offset + self.limit}"
        )
