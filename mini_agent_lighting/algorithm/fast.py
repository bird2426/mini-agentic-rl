...
from mini_agent_lighting.types import Attempt, Dataset, Rollout, RolloutStatus, Span
...
class Baseline(FastAlgorithm):
    ...
    def _span_to_string(self, rollout_id: str, attempt: Attempt, span: Span) -> str:
        ...
        elapsed = (
            f"{span.end_time - span.start_time:.2f}"
    ...
    async def _handle_rollout_finish(self, rollout: Rollout) -> None:
        ...
        try:
            ...
            logger.warning(
                "No adapter set for MockAlgorithm. Skipping trace adaptation."
            )
            adapter = None
    ...
    async def _enqueue_rollouts(
        ...
        for index in train_indices + val_indices:
            queuing_rollouts = await store.query_rollouts(
                status_in=["queuing", "requeuing"]
            )
            if len(queuing_rollouts) <= 1:
                ...
                rollout = await store.enqueue_rollout(
                    input=sample, mode=mode, resources_id=resources_id
                )
                logger.info(
                    f"[Rollout {rollout.rollout_id}] Enqueued in {mode} mode with sample: {sample}"
                )
            ...
            if rollout is not None:
                ...
                if last_status != rollout.status:
                    if last_status is not None:
                        logger.info(
                            f"[Rollout {rollout_id}] Status changed to {rollout.status}."
                        )
                    else:
                        logger.info(
                            f"[Rollout {rollout_id}] Status is initialized to {rollout.status}."
                        )
                    last_status = rollout.status

                else:
                    logger.debug(
                        f"[Rollout {rollout_id}] Status is still {rollout.status}."
                    )
    ...
    async def run(
        ...
        val_indices = list(
            range(train_dataset_length, train_dataset_length + val_dataset_length)
        )
        ...
        if initial_resources is not None:
            ...
            logger.warning(
                "No initial resources provided. Skip initializing resources."
            )
            resources_id = None
        ...
        for epoch in range(self.n_epochs):
                ...
                while True:
                    ...
                    if len(queuing_rollouts) <= self.max_queue_length:
                        ...
                        harvest_tasks.append(
                            asyncio.create_task(
                                self._harvest_rollout_spans(rollout.rollout_id)
                            )
                        )
                        logger.info(
            ...
            logger.info(
                f"Waiting for {len(harvest_tasks)} harvest tasks to complete..."
            )
