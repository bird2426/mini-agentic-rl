...
from mini_agent_lighting.adapter.messages import TraceToMessages
from mini_agent_lighting.algorithm.base import Algorithm
from mini_agent_lighting.algorithm.utils import (
    batch_iter_over_dataset,
    with_llm_proxy,
    with_store,
)
from mini_agent_lighting.reward import find_final_reward
...
if TYPE_CHECKING:
    from mini_agent_lighting.llm_proxy import LLMProxy
...
class APO(Algorithm, Generic[T_task]):
    ...
    def __init__(
        ...
        *,
        ...
        self.apply_edit_prompt_files = (
            apply_edit_prompt_files or APPLY_EDIT_PROMPT_FILES
        )
    ...
    def _create_versioned_prompt(
        ...
        *,
        ...
        return VersionedPromptTemplate(
            version=version, prompt_template=prompt_template, score=score
        )
    ...
    async def compute_textual_gradient(
        ...
        *,
        ...
        if len(rollout_results) < self.gradient_batch_size:
            ...
            sampled_rollout_results = random.sample(
                rollout_results, self.gradient_batch_size
            )
    ...
    async def evaluate_prompt_on_batch(
        ...
        *,
        ...
        for t in dataset:
            r = await store.enqueue_rollout(
                input=t, mode=mode, resources_id=resource_update.resources_id
            )
        ...
        while time.time() < deadline:
            finished = await store.wait_for_rollouts(
                rollout_ids=rollout_ids, timeout=0.0
            )
    ...
    def _initialize_beam(
        ...
        str, PromptTemplate, Iterator[Sequence[T_task]], Iterator[Sequence[T_task]]
    ]:
        """
        ...
        grad_dataset_iterator = batch_iter_over_dataset(
            train_dataset, self.gradient_batch_size
        )
    ...
    def _sample_parent_prompts(
        ...
        if len(beam) < self.beam_width:
            ...
            return [
                (i % len(beam), beam[i % len(beam)]) for i in range(self.beam_width)
            ]
    ...
    async def _generate_candidate_prompts(
        ...
        parent_prompts_str = [
            f"{p.version}:{p.score:.3f}" if p.score is not None else p.version
            for _, p in parent_prompts
        ]
        self._log(
            logging.INFO,
        ...
        for real_beam_idx, (beam_idx, prompt) in enumerate(parent_prompts):
            ...
            for branch_idx in range(self.branch_factor):
                ...
                baseline_score = (
                    f"{prompt.score:.3f}" if prompt.score is not None else "N/A"
                )
                self._log(
                    logging.INFO,
                ...
                new_prompt_template = PromptTemplate(
                    template=new_prompt, engine="f-string"
                )
    ...
    async def _evaluate_and_select_beam(
        ...
        sorted_prompts = [
            p
            for p in sorted(
                candidates, key=lambda x: cast(float, x.score), reverse=True
            )
        ]
        selected_prompts = sorted_prompts[: self.beam_width]
        selected_versions = [
            f"{prompt.version}:{prompt.score:.3f}"
    ...
    async def _update_best_prompt(
        ...
        display_round = round_num + 1
        best_prompt = beam[0]
        prefix = self._format_log_prefix(
            round_num=display_round, prompt_version=best_prompt.version
        )
    ...
    async def run(
        ...
        llm_proxy: Optional[
            LLMProxy
        ],  # Injected by decorator - callers should not provide this parameter
        ...
        if self.run_initial_validation:
            seed_prefix = self._format_log_prefix(
                round_num=0, prompt_version=seed_versioned.version
            )
            self._log(
                logging.INFO,
        ...
        for rnd in range(self.beam_rounds):
            ...
            beam = await self._evaluate_and_select_beam(
                all_candidates, resource_name, val_iterator, rnd
            )
