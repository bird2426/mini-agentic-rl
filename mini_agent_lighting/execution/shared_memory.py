...
from mini_agent_lighting.store.threading import LightningStoreThreaded
...
class SharedMemoryExecutionStrategy(ExecutionStrategy):
    ...
    async def _run_until_completed_or_canceled(
        self, coro: Awaitable[Any], stop_evt: ExecutionEvent
    ) -> Any:
        ...
        async def watcher() -> None:
            ...
            try:
                ...
                await asyncio.wait_for(
                    asyncio.shield(task), timeout=self.graceful_delay
                )  # type: ignore
                ...
                logger.debug(
                    "Bundle already canceled by someone else; exiting watcher."
                )
                return
        ...
        try:
            ...
            if not task.done():
                try:
                    await asyncio.wait_for(
                        task, timeout=self.graceful_delay
                    )  # second chance
                except asyncio.TimeoutError:
                    logger.error(
    ...
    def _run_algorithm(
        ...
        try:
            asyncio.run(
                self._run_until_completed_or_canceled(
                    algorithm(store, stop_evt), stop_evt
                )
            )
        except asyncio.CancelledError:
    ...
    def _run_runner(
        ...
        try:
            asyncio.run(
                self._run_until_completed_or_canceled(
                    runner(store, worker_id, stop_evt), stop_evt
                )
            )
        except asyncio.CancelledError:
            logger.info(
                "Runner bundle (worker_id=%s) canceled due to stop signal.", worker_id
            )
        except BaseException as exc:
            logger.exception(
                "Runner bundle crashed (worker_id=%s); signaling stop to others.",
                worker_id,
            )
            if thread_exceptions is not None:
                thread_exceptions.put(exc)
    ...
    def execute(
        ...
        def make_thread(
            name: str, target: Callable[..., Any], args: Tuple[Any, ...]
        ) -> threading.Thread:
        ...
        try:
            ...
            logger.warning(
                "KeyboardInterrupt received on main thread; initiating cooperative shutdown..."
            )
            stop_evt.set()
        finally:
