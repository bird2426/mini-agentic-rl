...
from mini_agent_lighting.env_var import (
    LightningEnvVar,
...
from mini_agent_lighting.store.client_server import (
...
class ClientServerExecutionStrategy(ExecutionStrategy):
    ...
    def __init__(
        ...
        Args:
            ...
            allowed_exit_codes: Allowed exit codes for subprocesses.
                ...
                by SIGTERM (-15).
        """
        resolved_role = resolve_str_env_var(
            LightningEnvVar.AGL_CURRENT_ROLE, override=role, fallback="both"
        )
        ...
        self.server_port = resolve_int_env_var(
            LightningEnvVar.AGL_SERVER_PORT, override=server_port, fallback=4747
        )
        ...
        if main_process == "runner":
            if self.role != "both":
                raise ValueError(
                    "main_process='runner' is only supported when role='both'"
                )
            if n_runners != 1:
    ...
    async def _execute_algorithm(
        ...
        if self.managed_store:
            logger.info(
                "Starting LightningStore server on %s:%s",
                self.server_host,
                self.server_port,
            )
            wrapper_store = LightningStoreServer(
                store, host=self.server_host, port=self.server_port
            )
        ...
        try:
            if self.managed_store and isinstance(wrapper_store, LightningStoreServer):
                ...
                server_started = True
                logger.debug(
                    "Algorithm bundle starting against endpoint %s",
                    wrapper_store.endpoint,
                )
            ...
            if (
                self.managed_store
                and isinstance(wrapper_store, LightningStoreServer)
                and server_started
            ):
                try:
    ...
    async def _execute_runner(
        ...
        try:
            if self.managed_store:
                logger.debug(
                    "Runner %s connecting to server at %s:%s",
                    worker_id,
                    self.server_host,
                    self.server_port,
                )
            ...
            logger.debug(
                "Runner %s received CancelledError; signaling stop event", worker_id
            )
            stop_evt.set()
            raise
        except KeyboardInterrupt:
            logger.warning(
                "Runner %s received KeyboardInterrupt; signaling stop event", worker_id
            )
            stop_evt.set()
            raise
        except BaseException:
            ...
            if self.managed_store and isinstance(client_store, LightningStoreClient):
                try:
                    ...
                    logger.exception(
                        "Error closing LightningStore client for runner %s", worker_id
                    )
                else:
        ...
        *,
        ...
        def _runner_sync(
            ...
            try:
                ...
                logger.warning(
                    "Runner (asyncio) %s received KeyboardInterrupt; exiting gracefully",
                    worker_id,
                )
            except BaseException as exc:
                logger.exception(
                    "Runner (asyncio) %s crashed by %s; signaling stop event",
                    worker_id,
                    exc,
                )
                raise
        ...
        for i in range(self.n_runners):
            process = cast(
                ...
                ctx.Process(
                    target=_runner_sync,
                    args=(runner, i, store, stop_evt),
                    name=f"runner-{i}",
                ),  # type: ignore
            )
            process.start()
            logger.debug(
                "Spawned runner process %s (pid=%s)", process.name, process.pid
            )
    ...
    def _spawn_algorithm_process(
        ...
        *,
        ...
        def _algorithm_sync(
            algorithm: AlgorithmBundle, store: LightningStore, stop_evt: ExecutionEvent
        ) -> None:
            try:
                asyncio.run(self._execute_algorithm(algorithm, store, stop_evt))
            except KeyboardInterrupt:
                logger.warning(
                    "Algorithm (asyncio.run) received KeyboardInterrupt; exiting gracefully"
                )
            except BaseException as exc:
                logger.exception(
                    "Algorithm (asyncio.run) crashed by %s; signaling stop event", exc
                )
                raise
        ...
        process = cast(
            multiprocessing.Process,
            ctx.Process(
                target=_algorithm_sync,
                args=(algorithm, store, stop_evt),
                name="algorithm",
            ),  # type: ignore
        )
        process.start()
    ...
    def _signal_processes(
        ...
        for process in processes:
            try:
                ...
                logger.exception(
                    "Error signaling process %s (pid=%s)", process.name, process.pid
                )
    ...
    def _shutdown_processes(
        ...
        self._signal_processes(
            alive, lambda p: os.kill(cast(int, p.pid), signal.SIGINT)
        )
        ...
        if alive:
            logger.error(
                "Subprocesses failed to exit even after kill(): %s",
                ", ".join(p.name or str(p.pid) for p in alive),
            )

    def _check_process_exitcodes(
        self, processes: Iterable[multiprocessing.Process]
    ) -> None:
        """Raise an error if any managed process exited with a non-zero status."""
        failed = [
            p for p in processes if p.exitcode not in self.allowed_exit_codes + (None,)
        ]
        if failed:
            formatted = ", ".join(
                f"{p.name or p.pid} (exitcode={p.exitcode})" for p in failed
            )
            raise RuntimeError(
                f"Subprocesses failed with unexpected exit codes: {formatted}"
            )

    def execute(
        self, algorithm: AlgorithmBundle, runner: RunnerBundle, store: LightningStore
    ) -> None:
        logger.info(
            ...
            if processes:
                try:
                    ...
                    if exception is not None or keyboard_interrupt:
                        ...
                        logger.warning(
                            "Subprocesses ended abnormally during shutdown: %s", err
                        )
                    else:
                        raise
