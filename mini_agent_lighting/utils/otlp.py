...
from mini_agent_lighting.types.tracer import (
    Attributes,
    Event,
    Link,
    OtelResource,
...
async def handle_otlp_export(
    ...
    if not body:
        ...
        try:
            ...
            return _bad_request_response(
                request, f"Unable to parse OTLP {signal_name} payload: {exc}"
            )
...
async def spans_from_proto(
    ...
    sequence_id_bulk_issuer: Callable[
        [Sequence[Tuple[str, str]]], Awaitable[Sequence[int]]
    ],
) -> List[Span]:
    ...
    for resource_spans in request.resource_spans:
        ...
        rollout_id_resource = resource_attrs.get(
            LightningResourceAttributes.ROLLOUT_ID.value
        )
        attempt_id_resource = resource_attrs.get(
        ...
        sequence_id_resource = resource_attrs.get(
            LightningResourceAttributes.SPAN_SEQUENCE_ID.value
        )

        otel_resource = _resource_from_proto(
            resource_spans.resource, getattr(resource_spans, "schema_url", "")
        )
        ...
        for scope_spans in resource_spans.scope_spans:
            for proto_span in scope_spans.spans:
                ...
                parent_id_hex = (
                    _bytes_to_span_id_hex(proto_span.parent_span_id)
                ...
                rollout_id_span = span_attrs.get(
                    LightningResourceAttributes.ROLLOUT_ID.value
                )
                attempt_id_span = span_attrs.get(
                    LightningResourceAttributes.ATTEMPT_ID.value
                )
                sequence_id_span = span_attrs.get(
                    LightningResourceAttributes.SPAN_SEQUENCE_ID.value
                )
                ...
                rollout_id_raw = (
                    rollout_id_span
                    if rollout_id_span is not None
                    else rollout_id_resource
                )
                attempt_id_raw = (
                    attempt_id_span
                    if attempt_id_span is not None
                    else attempt_id_resource
                )
                sequence_id_raw = (
                    sequence_id_span
                    if sequence_id_span is not None
                    else sequence_id_resource
                )

                rollout_id, attempt_id = _normalize_rollout_attempt_id(
                    rollout_id_raw, attempt_id_raw
                )
...
class LightningStoreOTLPExporter(OTLPSpanExporter):
    ...
    def enable_store_otlp(
        self, endpoint: str, rollout_id: str, attempt_id: str
    ) -> None:
    ...
    def export(self, spans: Sequence[ReadableSpan]) -> SpanExportResult:
        if self._rollout_id is not None and self._attempt_id is not None:
            ...
            logger.debug(
                "Rollout ID and Attempt ID not set; using default OTLP exporter behavior."
            )
            return super().export(spans)
        else:
...
def _maybe_gzip_response(
    ...
    tokens = [
        token.split(";")[0].strip().lower() for token in ae.split(",") if token.strip()
    ]
...
def _bad_request_response(
    request: Request, message: str, content_type: str = PROTOBUF_CT
) -> Response:
    """
...
def _any_value_to_python(value: AnyValue) -> Any:
    ...
    if kind == "kvlist_value":
        ...
        return {
            kv.key: _any_value_to_python(kv.value) for kv in value.kvlist_value.values
        }
    if kind == "bytes_value":
