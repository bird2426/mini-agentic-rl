...
async def shutdown_uvicorn_server(
    server: uvicorn.Server, task: asyncio.Task[None], timeout: float = 5.0
) -> None:
...
async def run_uvicorn_asyncio(
    ...
    async def _watch_server() -> None:
        ...
        while (
            time.time() < deadline
            and not uvicorn_server.started
            and server_start_exception is None
        ):
        ...
        if health_url is not None:
            ...
            async with aiohttp.ClientSession() as session:
                while time.time() < deadline:
                    try:
                        ...
                        logger.debug(
                            f"Error probing health endpoint {health_url}: {str(e)}"
                        )
                    await asyncio.sleep(0.1)
            ...
            logger.info(
                "Server does not provide a health check endpoint. Skipping health check."
            )
        ...
        async with serve_context:
            try:
                ...
                if wait_for_serve:
                    ...
                    logger.exception(
                        "Uvicorn server failed to serve. Inspect the logs for details."
                    )
...
def run_uvicorn_thread(
    ...
    try:
        ...
        logger.exception(
            "Exception within the thread server loop. Inspect the logs for details."
        )
    ...
    def _watchdog() -> None:
        ...
        while time.time() < deadline and not arbiter.WORKERS:  # type: ignore
            ...
            if runtime_error is not None:
                logger.error(
                    "Gunicorn arbiter exited during startup. Watchdog exiting."
                )
                return
            time.sleep(0.1)
        ...
        if not arbiter.WORKERS:  # type: ignore
            ...
            logger.error(
                "Gunicorn workers did not start within %.2f seconds.", elapsed_time
            )
        ...
        if health_url:
            while time.time() < deadline:
                ...
                if runtime_error is not None:
                    logger.error(
                        "Gunicorn arbiter exited during health check. Watchdog exiting."
                    )
                    return
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        logger.debug(
                            f"Server is healthy at {health_url} in {time.time() - start:.2f} seconds."
                        )
            ...
            if runtime_error is None:
                ...
                # If arbiter has thrown an exception, report it.
                logger.error(
                    "Gunicorn arbiter exited unexpectedly before health check. Watchdog exiting."
                )
...
class PythonServerLauncher:
    ...
    def __init__(
        ...
        serve_context: Optional[AsyncContextManager[Any]] = None,
    ):
    ...
    def initialize(self):
        ...
        self._uvicorn_task: Optional[asyncio.Task[None]] = (
            None  # returned by run_uvicorn_asyncio()
        )
        ...
        self._gunicorn_app: Optional[GunicornApp] = (
            None  # programmatic gunicorn wrapper
        )
    ...
    async def start(self):
        ...
        logger.info(
            f"Server {self._normalize_app_ref(self.app)} started at {self.endpoint}"
        )
    ...
    async def run_forever(self):
        ...
        if mode == "asyncio":
            ...
            try:
                ...
                logger.warning(
                    "Server received cancellation signal. Shutting down gracefully."
                )
                await self._stop_uvicorn_asyncio()
                raise
            ...
            try:
                ...
                logger.warning(
                    "Server thread received cancellation signal. Shutting down gracefully."
                )
                await self._stop_uvicorn_thread()
                raise
            ...
            try:
                ...
                logger.warning(
                    "Server process received cancellation signal. Shutting down gracefully."
                )
                await self._stop_serving_process()
                raise
    ...
    def _ensure_port(self) -> int:
        if self._port is None:
            logger.warning(
                "No port provided, using pick_unused_port to pick a random unused port."
            )
        ...
        if self._access_host is None:
            if self.args.access_host is None:
                if self._ensure_host() in ("0.0.0.0", "::"):
                    ...
                    logger.warning(
                        "No access host provided, using default outbound IPv4 address for this machine."
                    )
    ...
    async def _start_uvicorn_thread(self):
        ...
        timeout = (
            self.args.startup_timeout * 2
        )  # Allows twice the timeout for the thread to get the event
        try:
            evt: ChildEvent = await asyncio.to_thread(
                self._thread_event_queue.get, True, timeout
            )
        except queue.Empty:
            if not self._thread.is_alive():
                raise RuntimeError(
                    "Threaded server failed to start and is not alive. No error event was received."
                )
            logger.error(
            ...
            await self._stop_uvicorn_thread()
            raise RuntimeError(
                "Threaded server failed to start and sends no event. This should not happen."
            )

        if evt.kind == "error":
            logger.error(
                "Threaded server failed to start (%s): %s\n%s",
                evt.exc_type,
                evt.message,
                evt.traceback,
            )
            ...
            if self._thread.is_alive():
                logger.error(
                    "Threaded server failed to start and refused to shut down."
                )
    ...
    async def _start_serving_process(self):
        ...
        if self.args.n_workers > 1:
            ...
            if "PROMETHEUS_MULTIPROC_DIR" in os.environ:
                from mini_agent_lighting.utils.metrics import shutdown_metrics
        ...
        try:
            evt: ChildEvent = await asyncio.to_thread(
                self._mp_event_queue.get, True, timeout
            )
        except queue.Empty:
            if not self._proc.is_alive():
                raise RuntimeError(
                    "Server process failed to start and is not alive. No error event was received."
                )
            logger.error(
            ...
            await self._stop_serving_process()
            raise RuntimeError(
                "Server process failed to start and sends no event. This should not happen."
            )
        ...
        if self._proc is not None:
            ...
            if self._proc.is_alive():
                ...
                await asyncio.to_thread(
                    self._proc.join, 5.0
                )  # Use a constant timeout for SIGKILL

            if self._proc.is_alive():
                raise RuntimeError(
                    "Server process failed to shut down after SIGTERM and SIGKILL."
                )
        else:
            logger.info("Subprocess server was not running. Nothing to stop.")
