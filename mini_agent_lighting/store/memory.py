...
def estimate_model_size(obj: Any) -> int:
    ...
    if isinstance(obj, BaseModel):
        ...
        return sum(estimate_model_size(value) for value in values) + sys.getsizeof(
            cast(object, obj)
        )
    if isinstance(obj, MappingABC):
        mapping = cast(Mapping[Any, Any], obj)
        return sum(
            estimate_model_size(value) for value in mapping.values()
        ) + sys.getsizeof(cast(object, obj))
    if isinstance(obj, (list, tuple, set)):
        iterable = cast(Iterable[Any], obj)
        return sum(estimate_model_size(value) for value in iterable) + sys.getsizeof(
            cast(object, obj)
        )
...
class InMemoryLightningStore(
    CollectionBasedLightningStore[InMemoryLightningCollections]
):
    """
        ...
        *,
        ...
        super().__init__(
            collections=InMemoryLightningCollections(
                lock_type="thread" if thread_safe else "asyncio", tracker=tracker
            ),
            tracker=tracker,
        ...
        if not (0 <= self._safe_threshold_bytes < self._eviction_threshold_bytes):
            raise ValueError(
                "safe_memory_threshold must be smaller than eviction_memory_threshold"
            )
    ...
    async def add_resources(self, resources: NamedResources) -> ResourcesUpdate:
        ...
        async with self.collections.atomic(
            mode="rw", snapshot=self._read_snapshot, labels=["resources"]
        ):
    ...
    async def _post_update_rollout(
        ...
        async with self.collections.atomic(
            mode="rw", snapshot=self._read_snapshot, labels=["rollouts"]
        ):
            for rollout, _ in rollouts:
                ...
                if is_finished(rollout):
                    self._completion_events.setdefault(
                        rollout.rollout_id, aiologic.Event()
                    )
    ...
    async def _unlocked_query_rollouts_by_rollout_ids(
        ...
        rollouts = [
            await collections.rollouts.get({"rollout_id": {"exact": rollout_id}})
    ...
    async def _unlocked_get_running_rollouts(
        ...
        async with self.collections.atomic(
            ...
            rollouts = await self._unlocked_query_rollouts_by_rollout_ids(
                collections, list(self._running_rollout_ids)
            )
            ...
            for rollout in rollouts:
                ...
                if not latest_attempt:
                    ...
                    continue
                running_rollouts.append(
                    AttemptedRollout(**rollout.model_dump(), attempt=latest_attempt)
                )
        return running_rollouts

    @tracked(
        "query_spans_inmemory"
    )  # Since this method calls super, we need to track it separately
        ...
        if self._latest_resources_id is not None:
            async with self.collections.atomic(
                ...
                return await collections.resources.get(
                    filter={"resources_id": {"exact": self._latest_resources_id}}
                )
        return None
        ...
        *,
        ...
        if value is None:
            ...
            if minimum == 0:
                ...
                if not (0 < value <= 1):
                    raise ValueError(
                        f"{name} ratio must be greater than 0 and at most 1"
                    )
    ...
    async def _maybe_evict_spans(
        ...
        candidates: List[tuple[float, str]] = [
            (start_time, rollout_id)
            for rollout_id, start_time in self._start_time_by_rollout.items()
        ]
        candidates.sort()

        logger.info(
            f"Evicting spans for {len(candidates)} rollouts to free up memory..."
        )
        ...
        for _, rollout_id in candidates:
            if self._total_span_bytes <= self._safe_threshold_bytes:
                break
            logger.debug(
                f"Evicting spans for rollout {rollout_id} to free up memory..."
            )
            await self._evict_spans_for_rollout(collections, rollout_id)
        logger.info(
            f"Freed up {memory_consumed_before - self._total_span_bytes} bytes of memory"
        )

    @tracked("_evict_spans_for_rollout")
    async def _evict_spans_for_rollout(
        self, collections: InMemoryLightningCollections, rollout_id: str
    ) -> None:
