...
from mini_agent_lighting.types import Attempt, AttemptedRollout, Rollout
...
class MongoLightningStore(CollectionBasedLightningStore[MongoLightningCollections]):
    ...
    def __init__(
        ...
        *,
        ...
        if partition_id is None:
            ...
            logger.info(
                "No partition id provided, generated a new one: %s", partition_id
            )
    ...
    async def wait_for_rollouts(
        ...
        while deadline is None or current_time <= deadline:
            ...
            rest_time = (
                max(0.01, min(deadline - time.time() - 0.1, 10.0))
        ...
        return [
            finished_rollouts[rollout_id]
            for rollout_id in rollout_ids
            if rollout_id in finished_rollouts
        ]
    ...
    async def _unlocked_many_rollouts_to_attempted_rollouts(
        ...
        return [
            (
                AttemptedRollout(
                    **rollout.model_dump(), attempt=latest_attempts[rollout.rollout_id]
                )
