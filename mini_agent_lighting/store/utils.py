...
async def rollout_status_from_attempt(
    ...
    if (
        ...
        or attempt.status == "unresponsive"
    ):
...
async def scan_unhealthy_rollouts(
    ...
    for rollout in rollouts:
        ...
        if (
            ...
            and current_time - latest_attempt.start_time > config.timeout_seconds
        ):
        ...
        if (
            ...
            and current_time - latest_attempt.last_heartbeat_time
            > config.unresponsive_seconds
        ):
            updates[(latest_attempt.rollout_id, latest_attempt.attempt_id)] = (
                "unresponsive"
            )
            continue
