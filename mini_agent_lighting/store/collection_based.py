...
from mini_agent_lighting.utils.id import generate_id
from mini_agent_lighting.utils.metrics import MetricsBackend
...
_current_public_store_method: ContextVar[str] = ContextVar(
    "public_store_method", default=_UNKNOWN_STORE_METHOD
)
_current_private_store_method: ContextVar[str] = ContextVar(
    "private_store_method", default=_UNKNOWN_STORE_METHOD
)
...
def _with_collections_execute(labels: Sequence[AtomicLabels]):
    ...
    def decorator(
        func: Callable[
            Concatenate[SelfT, T_collections, P], CoroutineType[Any, Any, R]
        ],
...
def tracked(name: str):
    ...
    def decorator(func: T_callable) -> T_callable:
        ...
        @functools.wraps(func)
        async def wrapper(
            self: CollectionBasedLightningStore[T_collections],
            *args: Any,
            **kwargs: Any,
        ) -> Any:
            ...
            try:
                ...
                try:
                    ...
                    await self._tracker.inc_counter(  # pyright: ignore[reportPrivateUsage]
                        ...
                        labels={
                            "method": name,
                            "store_pubmeth": public_meth_in_stack,
                            "status": status,
                        },
                    )
...
def healthcheck_before(func: T_callable) -> T_callable:
    ...
    @functools.wraps(func)
    async def wrapper(
        self: CollectionBasedLightningStore[T_collections], *args: Any, **kwargs: Any
    ) -> Any:
...
class CollectionBasedLightningStore(LightningStore, Generic[T_collections]):
    ...
    async def _get_latest_resources(self) -> Optional[ResourcesUpdate]:
        ...
        async with self.collections.atomic(
            ...
            return await collections.resources.get(
                sort={"name": "update_time", "order": "desc"}
            )

    @tracked("_update_or_insert_worker")
    async def _update_or_insert_worker(
        self, worker: Worker, update_fields: Sequence[str] | None = None
    ) -> Worker:
        """Create a worker if it doesn't exist. Update its `update_fields` if it already exists."""
        async with self.collections.atomic(
            mode="rw", snapshot=self._read_snapshot, labels=["workers"]
        ) as collections:
            updated_workers = await collections.workers.upsert(
                [worker], update_fields=update_fields
            )
    ...
    async def _unlocked_sync_worker_with_attempt(
        ...
        if attempt.status in ("succeeded", "failed"):
            ...
            update_fields.extend(
                ["last_idle_time", "status", "current_rollout_id", "current_attempt_id"]
            )
            ...
            update_fields.extend(
                ["last_busy_time", "status", "current_rollout_id", "current_attempt_id"]
            )
    ...
    async def _sync_workers_with_attempts(
        self, attempts: Sequence[Attempt], dequeue: bool = False
    ) -> None:
        ...
        """
        async with self.collections.atomic(
            mode="w", snapshot=self._read_snapshot, labels=["workers"]
        ) as collections:
            for attempt in attempts:
                await self._unlocked_sync_worker_with_attempt(
                    collections, attempt, dequeue
                )
    ...
    async def _dequeue_mark_worker_idle(self, worker_id: str) -> None:
        ...
        async with self.collections.atomic(
            mode="r", snapshot=self._read_snapshot, labels=["workers"]
        ) as collections:
        ...
        if not worker or worker.status != "idle":
            ...
            await self._update_or_insert_worker(
                worker, update_fields=["status", "last_idle_time", "last_dequeue_time"]
            )
        else:
            # only update last_dequeue_time
            worker = Worker(worker_id=worker_id, last_dequeue_time=now)
            await self._update_or_insert_worker(
                worker, update_fields=["last_dequeue_time"]
            )
    ...
    async def start_rollout(
        ...
        rollout_config = (
            config.model_copy(deep=True) if config is not None else RolloutConfig()
        )
        ...
        if resources_id is None:
            ...
            resources_id = (
                latest_resources.resources_id if latest_resources is not None else None
            )
    ...
    async def _enqueue_many_rollouts(
    ...
    async def enqueue_many_rollouts(
        ...
        for request in rollouts:
            ...
            if resources_id is None:
                resources_id = (
                    latest_resources.resources_id
                    if latest_resources is not None
                    else None
                )
    ...
    async def _unlocked_query_rollouts_by_rollout_ids(
        ...
        if len(rollout_ids) == 0:
            ...
            rollout = await collections.rollouts.get(
                {"rollout_id": {"exact": rollout_ids[0]}}
            )
            return [rollout] if rollout is not None else []
        else:
            result = await collections.rollouts.query(
                {"rollout_id": {"within": rollout_ids}}
            )
            # Preserve the order of rollout_ids.
            result_dict = {rollout.rollout_id: rollout for rollout in result}
            return [
                result_dict[rollout_id]
                for rollout_id in rollout_ids
                if rollout_id in result_dict
            ]
    ...
    async def _post_dequeue_rollouts(
        ...
        rollouts = await self._unlocked_query_rollouts_by_rollout_ids(
            collections, rollout_ids
        )

        if not rollouts:
            logger.warning(
                f"No rollout found for rollout IDs: {rollout_ids}, skipping dequeuing"
            )
            return []
        ...
        for rollout in rollouts:
            ...
            if is_queuing(rollout):
                ...
                existing_attempts = await self._unlocked_query_attempts_for_rollout(
                    collections, rollout.rollout_id
                )
                ...
                dequeue_results.append(
                    (
                        AttemptedRollout(**rollout.model_dump(), attempt=attempt),
                        update_fields,
                    )
                )
    ...
    async def dequeue_rollout(
        ...
        while True:
            ...
            logger.debug(
                "Rollout ID %s has been dequeued by Worker ID %s", rollout_id, worker_id
            )

            post_dequeue_result = await self._post_dequeue_rollouts(
            ...
            if post_dequeue_result:
                ...
                if worker_id is not None:
                    await self._sync_workers_with_attempts(
                        [attempted_rollout.attempt], dequeue=True
                    )
                logger.debug(
                    "Rollout has been prepared for Worker ID %s: %s",
                    worker_id,
                    attempted_rollout,
                )
    ...
    async def dequeue_many_rollouts(
        ...
        if worker_id is not None:
            if dequeued_rollouts:
                ...
                await self._sync_workers_with_attempts(
                    [dequeued_rollouts[-1].attempt], dequeue=True
                )
            else:
                # Mark the current worker as idle
                await self._dequeue_mark_worker_idle(worker_id)
    ...
    async def query_rollouts(
        ...
        *,
        ...
        if status_in is not None:
            ...
            warnings.warn(
                "status is deprecated, use status_in instead",
                DeprecationWarning,
                stacklevel=3,
            )
        ...
        if rollout_id_in is not None:
            ...
            warnings.warn(
                "rollout_ids is deprecated, use rollout_id_in instead",
                DeprecationWarning,
                stacklevel=3,
            )
        ...
        async with self.collections.atomic(
            ...
            attempted_rollouts = (
                await self._unlocked_many_rollouts_to_attempted_rollouts(
                    collections, rollouts.items
                )
            )

        return PaginatedResult(
            items=attempted_rollouts,
            limit=rollouts.limit,
            offset=rollouts.offset,
            total=rollouts.total,
        )

    @tracked("_unlocked_query_attempts_for_rollout")
    async def _unlocked_query_attempts_for_rollout(
    ...
    async def _unlocked_rollout_to_attempted_rollout(
        ...
        """
        latest_attempt = await self._unlocked_get_latest_attempt(
            collections, rollout.rollout_id
        )
    ...
    async def _unlocked_many_rollouts_to_attempted_rollouts(
        ...
        return [
            await self._unlocked_rollout_to_attempted_rollout(collections, rollout)
            for rollout in rollouts
        ]

    @tracked("_unlocked_get_latest_attempt")
    async def _unlocked_get_latest_attempt(
        self, collections: T_collections, rollout_id: str
    ) -> Optional[Attempt]:
    ...
    async def query_resources(
        ...
        *,
        ...
        if resources_id is not None:
            resources_id_field = cast(
                FilterField, filters.setdefault("resources_id", {})
            )
    ...
    async def add_resources(self, resources: NamedResources) -> ResourcesUpdate:
        ...
        async with self.collections.atomic(
            mode="w", snapshot=self._read_snapshot, labels=["resources"]
        ) as collections:
    ...
    async def update_resources(
        ...
        """
        current_time = time.time()
        existing = await collections.resources.get(
            {"resources_id": {"exact": resources_id}}
        )
        if existing is None:
            update = ResourcesUpdate(
                resources_id=resources_id,
    ...
    async def _issue_many_span_sequence_ids(self, rollout_ids: List[str]) -> List[int]:
        ...
        for rollout_id, count in request_counts.items():
            async with self.collections.atomic(
                mode="rw", snapshot=False, labels=["span_sequence_ids"]
            ) as collections:
                latest_values[rollout_id] = await collections.span_sequence_ids.inc(
                    rollout_id, count
                )

        next_value_tracker: Dict[str, int] = {
            rollout_id: latest_values[rollout_id] - request_counts[rollout_id]
            for rollout_id in request_counts
        }
    ...
    async def get_many_span_sequence_ids(
        ...
        return await self._issue_many_span_sequence_ids(
            [rollout_id for rollout_id, _ in rollout_attempt_ids]
        )
    ...
    async def add_span(self, span: Span) -> Optional[Span]:
        ...
        successful_spans = await self._add_many_spans_helper(
            span.rollout_id, span.attempt_id, [span]
        )
    ...
    async def add_many_spans(self, spans: Sequence[Span]) -> Sequence[Span]:
        ...
        for (rollout_id, attempt_id), spans in spans_by_rollout_attempt.items():
            await self._sync_span_sequence_id(
                rollout_id, max(span.sequence_id for span in spans)
            )
    ...
    async def _insert_spans_with_fallback(
    ...
    async def _add_many_spans_helper(
        ...
        async with self.collections.atomic(
            ...
            if not current_attempt:
                raise ValueError(
                    f"Attempt {attempt_id} not found for rollout {rollout_id}"
                )
        ...
        logger.debug(
            "Added %d spans for rollout %s, attempt %s",
            len(successful_spans),
            rollout_id,
            attempt_id,
        )
    ...
    async def _post_add_spans(
        self, spans: Sequence[Span], rollout_id: str, attempt_id: str
    ) -> None:
        ...
        rollout_update = await self._on_attempt_heartbeat(
    ...
    async def _on_attempt_heartbeat(
        ...
        await collections.attempts.update(
            [attempt], update_fields=["last_heartbeat_time", "status"]
        )
    ...
    async def wait_for_rollouts(
        ...
        rollouts = await asyncio.gather(
            *[self.wait_for_rollout(rid, timeout) for rid in rollout_ids],
    ...
    async def wait_for_rollout(
        ...
        while deadline is None or time.time() < deadline:
            ...
            rest_time = (
                max(0.01, min(deadline - time.time(), 10.0))
        ...
        *,
        ...
        if attempt_id is None:
            ...
            if not latest_attempt:
                logger.debug(
                    f"No attempts found for rollout {rollout_id} when querying latest spans"
                )
        ...
        async with self.collections.atomic(
            mode="r", snapshot=self._read_snapshot, labels=["spans"]
        ) as collections:
    ...
    async def update_rollout(
        ...
        """
        async with self.collections.atomic(
            mode="w", snapshot=self._read_snapshot, labels=["rollouts"]
        ) as collections:
            rollout, update_fields = await self._unlocked_update_rollout_only(
    ...
    async def _unlocked_update_rollout_only(
        ...
        if not isinstance(status, Unset) and status in [
            "failed",
            "succeeded",
            "cancelled",
        ]:
        ...
        rollouts_updated = await collections.rollouts.update(
            [rollout_obj], update_fields=update_fields
        )
    ...
    async def _post_update_rollout(
        self,
        rollouts: Sequence[Tuple[Rollout, Sequence[str]]],
            ...
            if candidate_requeue_rollouts:
                ...
                if candidate_requeue_rollouts:
                    async with self.collections.atomic(
                        ...
                        await collections.rollout_queue.enqueue(
                            candidate_requeue_rollouts
                        )
        ...
        if attempt.attempt_id == latest_attempt.attempt_id:
            ...
            if rollout_status != rollout.status:
                (
                    updated_rollout,
                    update_fields,
                ) = await self._unlocked_update_rollout_only(
    ...
    async def update_worker(
        ...
        return await self._update_or_insert_worker(
            new_worker, update_fields=update_fields
        )

    @tracked("_unlocked_get_running_rollouts")
    async def _unlocked_get_running_rollouts(
        ...
        """
        filtered_rollouts = await collections.rollouts.query(
            filter={"status": {"within": ["preparing", "running"]}}
        )
        running_rollouts = await self._unlocked_many_rollouts_to_attempted_rollouts(
            collections, filtered_rollouts
        )
        ...
        for rollout in running_rollouts:
            if not isinstance(rollout, AttemptedRollout):
                logger.error(
                    f"Rollout {rollout.rollout_id} is running but has no attempts"
                )
                continue
    ...
    async def _scan_for_unhealthy_rollouts(self) -> None:
        ...
        (
            rollouts,
            attempts_sync_required,
        ) = await self._find_and_update_unhealthy_rollouts()
    ...
    async def _should_scan_for_unhealthy_rollouts(self) -> bool:
        ...
        async with self.collections.atomic(
            mode="rw", snapshot=self._read_snapshot, labels=["generic"]
        ):
            now = time.time()
    ...
    async def _find_and_update_unhealthy_rollouts(
        ...
        for (rollout_id, attempt_id), status in candidate_updates.items():
            ...
            ) = await self._unlocked_update_attempt_and_rollout(
...
COLLECTION_STORE_PUBLIC_METHODS = frozenset(
    [name for name in LightningStore.__dict__ if not name.startswith("_")]
    + ["_scan_for_unhealthy_rollouts"]
)

COLLECTION_STORE_ALL_METHODS = frozenset(
    [name for name in CollectionBasedLightningStore.__dict__]
)
