    ...
    for field_name, ops in filters.items():
        ...
        for op_name, expected in ops.items():
            ...
            if op_name == "exact":
                ...
                raise ValueError(
                    f"Unsupported filter operator '{op_name}' for field '{field_name}'"
                )

    return (
        all(all_conditions_match)
        if filter_logic == "and"
        else any(all_conditions_match)
    )
...
def _get_sort_value(item: object, sort_by: str) -> Any:
    ...
    if value is None:
        ...
        if sort_by not in model_fields:
            raise ValueError(
                f"Failed to sort items by '{sort_by}': field does not exist "
                f"on {item.__class__.__name__}"
            )
        ...
        if "float" in field_type_str:
            return 0.0
        raise ValueError(
            f"Failed to sort items by '{sort_by}': unsupported field type {field_type_str!r}"
        )
...
class ListBasedCollection(Collection[T]):
    ...
    def __repr__(self) -> str:
        return (
            f"<{self.__class__.__name__}[{self.item_type().__name__}] ({self._size})>"
        )
    ...
    def _ensure_item_type(self, item: T) -> None:
        ...
        if not isinstance(item, self._item_type):
            raise TypeError(
                f"Expected item of type {self._item_type.__name__}, "
    ...
    def _render_key_values(self, key_values: Sequence[Any]) -> str:
        return ", ".join(
            f"{name}={value!r}" for name, value in zip(self._primary_keys, key_values)
        )
        ...
        for idx, value in enumerate(key_values):
            ...
            if value not in current:
                if not create_missing:
                    raise KeyError(
                        f"Path does not exist for given primary keys: {self._render_key_values(key_values)}"
                    )
                current[value] = {}
            next_node = current[value]  # type: ignore
            if not isinstance(next_node, dict):
                raise ValueError(
                    f"Internal structure corrupted: expected dict, got {type(next_node)!r}"
                )  # type: ignore
    ...
    def _mutate_single(
        self, item: T, mode: MutationMode, update_fields: Sequence[str] | None = None
    ) -> Optional[T]:
            ...
            for value in node.values():
                ...
                if isinstance(value, self._item_type):
                    if _item_matches_filters(
                        value, filters, filter_logic, must_filters
                    ):
                        yield value
    ...
    def _iter_matching_items(
        ...
        if filter_logic != "and" and must_filters is None:
            return self._iter_items(
                filters=filters, must_filters=must_filters, filter_logic=filter_logic
            )
        ...
        for pk in self._primary_keys:
            ...
            for ops in combined_ops:
                ...
                if exact_value is not None and candidate != exact_value:
                    ...
                    logger.warning(
                        f"Contradictory exact filters for field '{pk}': {exact_value} != {candidate}"
                    )
                    return ()
        ...
        try:
            if len(pk_values_prefix) == len(self._primary_keys):
                ...
                parent, final_key = self._locate_node(
                    pk_values_prefix, create_missing=False
                )
    ...
    async def query(
        ...
        items_iter: Iterable[T] = self._iter_matching_items(
            filters, must_filters, filter_logic
        )
    ...
    async def update(
        ...
        for item in items:
            updated = self._mutate_single(
                item, mode="update", update_fields=update_fields
            )
            if updated is None:
                raise RuntimeError(
                    f"_mutate_single returned None for item {item}. This should never happen."
                )
    ...
    async def upsert(
        ...
        for item in items:
            upserted = self._mutate_single(
                item, mode="upsert", update_fields=update_fields
            )
            if upserted is None:
                raise RuntimeError(
                    f"_mutate_single returned None for item {item}. This should never happen."
...
class InMemoryLightningCollections(LightningCollections):
    ...
    def __init__(
        self,
        lock_type: Literal["thread", "asyncio"],
        tracker: MetricsBackend | None = None,
    ):
        super().__init__(tracker=tracker)
        self._lock: Mapping[
            AtomicLabels, _LoopAwareAsyncLock | _ThreadSafeAsyncLock
        ] = {
            "rollouts": _LoopAwareAsyncLock()
            if lock_type == "asyncio"
            else _ThreadSafeAsyncLock(),
        ...
        self._attempts = ListBasedCollection(
            items=[],
            item_type=Attempt,
            primary_keys=["rollout_id", "attempt_id"],
            id="attempts",
            tracker=tracker,
        )
        self._spans = ListBasedCollection(
            items=[],
            item_type=Span,
            primary_keys=["rollout_id", "attempt_id", "span_id"],
            id="spans",
            tracker=tracker,
        )
        self._resources = ListBasedCollection(
            items=[],
            item_type=ResourcesUpdate,
            primary_keys=["resources_id"],
            id="resources",
            tracker=tracker,
        )
        self._workers = ListBasedCollection(
            items=[],
            item_type=Worker,
            primary_keys=["worker_id"],
            id="workers",
            tracker=tracker,
        )
        self._rollout_queue = DequeBasedQueue(
            items=[], item_type=str, id="rollout_queue", tracker=tracker
        )
