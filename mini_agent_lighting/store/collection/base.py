...
from mini_agent_lighting.store.utils import LATENCY_BUCKETS
...
class TrackedCollection:
    ...
    async def tracking_context(self, operation: str, collection: str):
        ...
        if self._tracker is None:
            ...
            from mini_agent_lighting.store.collection_based import (
                get_current_store_methods,
            )
...
class LightningCollections(TrackedCollection):
    ...
    def register_collection_metrics(
        self, extra_labels: Optional[Sequence[str]] = None
    ) -> None:
        if self._tracker is None:
            return
        labels = [
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        async with self.atomic(
            mode=mode, snapshot=snapshot, commit=commit, labels=labels, **kwargs
        ) as collections:
...
def merge_must_filters(
    target: MutableMapping[str, FilterField], definition: Any
) -> None:
    ...
    if isinstance(definition, Mapping):
        ...
        for entry in definition:  # type: ignore
            if not isinstance(entry, Mapping):
                raise TypeError(
                    "Each `_must` entry must be a mapping of field names to operators"
                )
            entries.append(cast(Mapping[str, FilterField], entry))
    else:
        raise TypeError(
            "`_must` filters must be provided as a mapping or sequence of mappings"
        )
        ...
        for field_name, ops in entry.items():
            ...
            for op_name, expected in ops.items():
                if op_name in merged_ops:
                    raise ValueError(
                        f"Duplicate operator '{op_name}' for field '{field_name}' in must filters"
                    )
