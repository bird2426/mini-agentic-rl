...
def resolve_mongo_error_type(exc: BaseException | None) -> str | None:
    is_transient = isinstance(exc, PyMongoError) and exc.has_error_label(
...
def _field_ops_to_conditions(
    field: str, ops: Mapping[str, Any]
) -> List[Dict[str, Any]]:
    ...
    for op_name, raw_value in ops.items():
        if op_name == "exact":
            if raw_value is None:
                logger.debug(
                    f"Skipping exact filter for field '{field}' with None value"
                )
                continue
            ...
            if raw_value is None:
                logger.debug(
                    f"Skipping within filter for field '{field}' with None value"
                )
                continue
            try:
                ...
                raise ValueError(
                    f"Invalid iterable for within filter for field '{field}': {raw_value!r}"
                ) from exc
            conditions.append({field: {"$in": iterable}})
        elif op_name == "contains":
            if raw_value is None:
                logger.debug(
                    f"Skipping contains filter for field '{field}' with None value"
                )
                continue
            value = str(raw_value)
            ...
            raise ValueError(
                f"Unsupported filter operator '{op_name}' for field '{field}'"
            )
...
async def _ensure_collection(
    ...
    try:
        ...
        logger.debug(
            f"Collection '{collection_name}' may have already existed. No need to create it: {exc!r}"
        )
    except OperationFailure as exc:
        logger.debug(
            f"Failed to create collection '{collection_name}'. Probably already exists: {exc!r}"
        )
        # Some servers use OperationFailure w/ specific codes for "NamespaceExists"
        if exc.code in (
            48,
            68,
        ):  # 48: NamespaceExists, 68: already exists on older versions
            pass
        else:
            raise
    ...
    if primary_keys:
        ...
        try:
            await db[collection_name].create_index(
                keys, name=f"uniq_partition_{'_'.join(primary_keys)}", unique=True
            )
        except OperationFailure as exc:
            logger.debug(
                f"Index for collection '{collection_name}' already exists. No need to create it: {exc!r}"
            )
    ...
    if extra_indexes:
        for index in extra_indexes:
            try:
                await db[collection_name].create_index(
                    index, name=f"idx_{'_'.join(index)}"
                )
            except OperationFailure as exc:
                logger.debug(
                    f"Index for collection '{collection_name}' already exists. No need to create it: {exc!r}"
...
class MongoClientPool(Generic[T_mapping]):
    ...
    def __init__(
        self, *, mongo_uri: str, mongo_client_kwargs: Mapping[str, Any] | None = None
    ):
        ...
        self._collection_pool: Dict[
            Tuple[int, str, str], AsyncCollection[T_mapping]
        ] = {}
    ...
    async def get_client(self) -> AsyncMongoClient[T_mapping]:
        ...
        if existing is not None:
            await (
                existing.aconnect()
            )  # This actually does nothing if the client is already connected.
            return existing
        ...
        async with self._get_client_lock:
            ...
            client = AsyncMongoClient[T_mapping](
                self._mongo_uri, **self._mongo_client_kwargs
            )
...
class MongoBasedCollection(Collection[T_model]):
    ...
    def __init__(
        ...
        if not issubclass(item_type, BaseModel):  # type: ignore
            raise ValueError(
                f"item_type must be a subclass of BaseModel, got {item_type.__name__}"
            )
    ...
    async def ensure_collection(self) -> AsyncCollection[Mapping[str, Any]]:
        ...
        return await self._client_pool.get_collection(
    ...
    async def size(self) -> int:
        ...
        return await collection.count_documents(
            {"partition_id": self._partition_id}, session=self._session
        )
    ...
    def _render_pk_values(self, values: Sequence[Any]) -> str:
        return ", ".join(
            f"{pk}={value!r}" for pk, value in zip(self._primary_keys, values)
        )

    def _ensure_item_type(self, item: T_model) -> None:
        if not isinstance(item, self._item_type):
            raise TypeError(
                f"Expected item of type {self._item_type.__name__}, got {type(item).__name__}"
            )

    def _inject_partition_filter(
        self, filter: Optional[FilterOptions]
    ) -> Dict[str, Any]:
        ...
        elif isinstance(existing_must, Sequence) and not isinstance(
            existing_must, (str, bytes)
        ):
            combined["_must"] = [*existing_must, partition_must]
        else:
    ...
    async def query(
        ...
        if limit == 0:
            return PaginatedResult[T_model](
                items=[], limit=0, offset=offset, total=total
            )
    ...
    async def get(
        ...
        raw = await collection.find_one(
            mongo_filter, sort=mongo_sort, session=self._session
        )
    ...
    async def insert(self, items: Sequence[T_model]) -> None:
        ...
        try:
            async with self.tracking_context(
                "insert.insert_many", self._collection_name
            ):
    ...
    @tracked("update")
    async def update(
        self, items: Sequence[T_model], update_fields: Sequence[str] | None = None
    ) -> List[T_model]:
        if not items:
            return []
        ...
        for item in items:
            ...
            if update_fields is None:
                async with self.tracking_context(
                    "update.find_one_and_replace", self._collection_name
                ):
            ...
            # Branch 2: Partial Update
                ...
                update_doc = {
                    field: doc[field] for field in update_fields if field in doc
                }
                async with self.tracking_context(
                    "update.find_one_and_update", self._collection_name
                ):
    ...
    async def upsert(
        ...
        for item in items:
            ...
            target_fields = (
                update_fields if update_fields is not None else list(insert_doc.keys())
            )

            # 1. $set: Fields that should be overwritten if the document exists
            update_subset = {
                field: insert_doc[field]
                for field in target_fields
                if field in insert_doc
            }
            ...
            set_on_insert = {
                k: v for k, v in insert_doc.items() if k not in update_subset
            }
            ...
            async with self.tracking_context(
                "upsert.find_one_and_update", self._collection_name
            ):
            ...
            if result_doc is None:  # pyright: ignore[reportUnnecessaryComparison]
                raise RuntimeError(
                    f"Upsert resulted in no document for filter: {pk_filter}"
                )
    ...
    async def delete(self, items: Sequence[T_model]) -> None:
        ...
        for item in items:
            ...
            pk_filter = self._pk_filter(item)
            async with self.tracking_context(
                "delete.delete_one", self._collection_name
            ):
...
class MongoBasedQueue(Queue[T_generic], Generic[T_generic]):
    ...
    async def dequeue(self, limit: int = 1) -> Sequence[T_generic]:
        ...
        for _ in range(limit):
            async with self.tracking_context(
                "dequeue.find_one_and_update", self.collection_name
            ):
...
class MongoBasedKeyValue(KeyValue[K, V], Generic[K, V]):
    ...
    async def ensure_collection(
        self, *, create_indexes: bool = True
    ) -> AsyncCollection[Mapping[str, Any]]:
    ...
    async def set(self, key: K, value: V) -> None:
        ...
        try:
            ...
            raise DuplicatedPrimaryKeyError(
                "Duplicate key error while setting key-value item"
            ) from exc
    ...
    async def inc(self, key: K, amount: V) -> V:
        ...
        try:
            async with self.tracking_context(
                "inc.find_one_and_update", self.collection_name
            ):
                doc = await collection.find_one_and_update(
                    {
                        "partition_id": self._partition_id,
        ...
        if doc is None:  # type: ignore
            raise RuntimeError(
                "Failed to increment value; MongoDB did not return a document"
            )
        raw_value = doc["value"]
        return self._value_adapter.validate_python(raw_value)
    ...
    async def chmax(self, key: K, value: V) -> V:
        ...
        try:
            async with self.tracking_context(
                "chmax.find_one_and_update", self.collection_name
            ):
                doc = await collection.find_one_and_update(
                    {
                        "partition_id": self._partition_id,
        ...
        if doc is None:  # type: ignore
            raise RuntimeError(
                "Failed to update value; MongoDB did not return a document"
            )
        raw_value = doc["value"]
        return self._value_adapter.validate_python(raw_value)
...
class MongoLightningCollections(LightningCollections):
    ...
    def __init__(
        ...
        self._lock = (
            aiologic.Lock()
        )  # used for generic atomic operations like scan debounce seconds
        self._rollouts = (
            rollouts
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        async with client.start_session() as session:
            ...
            try:
                async with self._lock_manager(labels):
                    return await self.with_transaction(
                        session, collections, callback, read_concern, write_concern
                    )
            except (ConnectionFailure, OperationFailure) as exc:
                # Un-retryable errors.
                raise RuntimeError(
                    "Transaction failed with connection or operation error"
                ) from exc
    ...
    async def with_transaction(
        ...
        while True:
            ...
            try:
                ...
                async with self.tracking_context(
                    "execute.callback", self.collection_name
                ):
            ...
            async with self.tracking_context("execute.commit", self.collection_name):
                ...
                while True:
                    ...
                    try:
                        async with self.tracking_context(
                            "execute.commit_once", self.collection_name
                        ):
                        ...
                        if (
                            ...
                            and _within_time_limit()
                            and not (
                                isinstance(exc, OperationFailure) and exc.code == 50
                            )  # max_time_expired_error
                        ):
                            # Retry the commit.
                            await _jitter_before_retry()
                            continue
