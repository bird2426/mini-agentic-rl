...
from mini_agent_lighting.types import (
    Attempt,
    AttemptedRollout,
    AttemptStatus,
...
from mini_agent_lighting.utils.metrics import MetricsBackend, get_prometheus_registry
from mini_agent_lighting.utils.otlp import handle_otlp_export, spans_from_proto
from mini_agent_lighting.utils.server_launcher import (
    LaunchMode,
...
class LightningStoreServer(LightningStore):
    ...
    def __init__(
        ...
        if launcher_args is not None:
            if host is not None or port is not None or launch_mode != "thread":
                raise ValueError(
                    "host, port, and launch_mode cannot be set when launcher_args is provided."
                )
        ...
        if not store_capabilities.get("async_safe", False):
            raise ValueError(
                "The store is not async-safe. Please use another store for the server."
            )
        if self.launcher_args.launch_mode == "mp" and not store_capabilities.get(
            "zero_copy", False
        ):
            raise ValueError(
                "The store does not support zero-copy. Please use another store, or use asyncio or thread mode to launch the server."
            )
        if self.launcher_args.launch_mode == "thread" and not store_capabilities.get(
            "thread_safe", False
        ):
    ...
    async def start(self):
        ...
        end_time = time.time()
        server_logger.info(
            f"Lightning store server started in {end_time - start_time:.2f} seconds"
        )
    ...
    def _setup_routes(self):
        ...
        async def _log_time(  # pyright: ignore[reportUnusedFunction]
            ...
            if not request.url.path.startswith(
                API_V1_AGL_PREFIX
            ) and not request.url.path.startswith(API_V1_PREFIX + "/traces"):
                return await call_next(request)
        ...
        def _validate_paginated_request(
            ...
            if (
                request.sort_by is not None
                and request.sort_by not in target_type.model_fields
            ):
                raise HTTPException(
                    status_code=400,
            ...
            if request.sort_order not in ["asc", "desc"]:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid sort_order: {request.sort_order}, allowed values are: asc, desc",
                )
            if request.limit == 0 or (request.limit < 0 and request.limit != -1):
                raise HTTPException(
                    status_code=400,
                    detail="Limit must be greater than 0 or -1 for no limit",
                )
            if not request.offset >= 0:
                raise HTTPException(
                    status_code=400, detail="Offset must be greater than or equal to 0"
                )
            if hasattr(request, "filter_logic") and request.filter_logic not in [
                "and",
                "or",
            ]:  # type: ignore
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid filter_logic: {request.filter_logic}, allowed values are: and, or",  # type: ignore
                )
        ...
        def _build_paginated_response(
            ...
            return PaginatedResult(
                items=items, limit=limit, offset=offset, total=len(items)
            )
        ...
        @api.post(
            API_AGL_PREFIX + "/queues/rollouts/enqueue",
            status_code=201,
        ...
        @api.post(
            API_AGL_PREFIX + "/queues/rollouts/dequeue",
            response_model=List[AttemptedRollout],
        )
        async def dequeue_rollouts(  # pyright: ignore[reportUnusedFunction]
            ...
            rollouts = await self.dequeue_many_rollouts(
                limit=payload.limit, worker_id=payload.worker_id
            )
            return list(rollouts)

        @api.post(
            API_AGL_PREFIX + "/rollouts",
            status_code=201,
            response_model=AttemptedRollout,
        )
        ...
        @api.get(
            API_AGL_PREFIX + "/rollouts",
            response_model=PaginatedResult[Union[AttemptedRollout, Rollout]],
        )
        ...
        @api.post(
            API_AGL_PREFIX + "/rollouts/search",
            response_model=PaginatedResult[Union[AttemptedRollout, Rollout]],
        )
        async def search_rollouts(request: QueryRolloutsRequest):  # pyright: ignore[reportUnusedFunction]
            ...
            status_in = (
                request.status_in if "status_in" in request.model_fields_set else None
            )
            rollout_id_in = (
                request.rollout_id_in
                if "rollout_id_in" in request.model_fields_set
                else None
            )
            # Get all rollouts from the underlying store
            results = await self.query_rollouts(
            ...
            return _build_paginated_response(
                results, limit=request.limit, offset=request.offset
            )

        @api.get(
            API_AGL_PREFIX + "/rollouts/{rollout_id}",
            response_model=Union[AttemptedRollout, Rollout],
        )
        ...
        def _get_mandatory_field_or_unset(request: BaseModel | None, field: str) -> Any:
            ...
            if field in request.model_fields_set:
                ...
                if value is None:
                    raise HTTPException(
                        status_code=400,
                        detail=f"{field} is invalid; it cannot be a null value.",
                    )
                return value
            else:
                return UNSET
        ...
        async def update_rollout(  # pyright: ignore[reportUnusedFunction]
            ...
            return await self.update_rollout(
                ...
                resources_id=request.resources_id
                if "resources_id" in request.model_fields_set
                else UNSET,
        ...
        @api.post(
            API_AGL_PREFIX + "/rollouts/{rollout_id}/attempts",
            status_code=201,
            response_model=AttemptedRollout,
        )
        ...
        @api.post(
            API_AGL_PREFIX + "/rollouts/{rollout_id}/attempts/search",
            response_model=PaginatedResult[Attempt],
        )
        async def search_attempts(  # pyright: ignore[reportUnusedFunction]
            ...
            return _build_paginated_response(
                attempts, limit=request.limit, offset=request.offset
            )

        @api.post(
            API_AGL_PREFIX + "/rollouts/{rollout_id}/attempts/{attempt_id}",
        ...
        async def query_workers(params: QueryWorkersRequest = Depends()):  # pyright: ignore[reportUnusedFunction]
            ...
            return _build_paginated_response(
                workers, limit=params.limit, offset=params.offset
            )

        @api.post(
            API_AGL_PREFIX + "/workers/search", response_model=PaginatedResult[Worker]
        )
        async def search_workers(request: QueryWorkersRequest):  # pyright: ignore[reportUnusedFunction]
            ...
            return _build_paginated_response(
                workers, limit=request.limit, offset=request.offset
            )

        @api.get(
            API_AGL_PREFIX + "/workers/{worker_id}", response_model=Optional[Worker]
        )
        ...
        async def update_worker(  # pyright: ignore[reportUnusedFunction]
            ...
            return await self.update_worker(
                worker_id=worker_id,
                heartbeat_stats=_get_mandatory_field_or_unset(
                    request, "heartbeat_stats"
                ),
            )
        ...
        async def query_attempts(  # pyright: ignore[reportUnusedFunction]
            ...
            return _build_paginated_response(
                attempts, limit=params.limit, offset=params.offset
            )

        @api.get(
            API_AGL_PREFIX + "/rollouts/{rollout_id}/attempts/latest",
            response_model=Optional[Attempt],
        )
        ...
        @api.get(
            API_AGL_PREFIX + "/resources",
            response_model=PaginatedResult[ResourcesUpdate],
        )
        async def query_resources(params: QueryResourcesRequest = Depends()):  # pyright: ignore[reportUnusedFunction]
            ...
            return _build_paginated_response(
                resources, limit=params.limit, offset=params.offset
            )

        @api.post(
            API_AGL_PREFIX + "/resources",
            status_code=201,
            response_model=ResourcesUpdate,
        )
        ...
        @api.get(
            API_AGL_PREFIX + "/resources/latest",
            response_model=Optional[ResourcesUpdate],
        )
        async def get_latest_resources():  # pyright: ignore[reportUnusedFunction]
            return await self.get_latest_resources()

        @api.post(
            API_AGL_PREFIX + "/resources/{resources_id}", response_model=ResourcesUpdate
        )
        ...
        @api.get(
            API_AGL_PREFIX + "/resources/{resources_id}",
            response_model=Optional[ResourcesUpdate],
        )
        ...
        @api.post(
            API_AGL_PREFIX + "/spans", status_code=201, response_model=Optional[Span]
        )
        ...
        async def query_spans(params: QuerySpansRequest = Depends()):  # pyright: ignore[reportUnusedFunction]
            ...
            return _build_paginated_response(
                spans, limit=params.limit, offset=params.offset
            )

        @api.post(
            API_AGL_PREFIX + "/spans/search", response_model=PaginatedResult[Span]
        )
        async def search_spans(request: QuerySpansRequest):  # pyright: ignore[reportUnusedFunction]
            ...
            return _build_paginated_response(
                spans, limit=request.limit, offset=request.offset
            )
        ...
        async def get_next_span_sequence_id(request: NextSequenceIdRequest):  # pyright: ignore[reportUnusedFunction]
            sequence_id = await self.get_next_span_sequence_id(
                request.rollout_id, request.attempt_id
            )
        ...
        async def wait_for_rollouts(request: WaitForRolloutsRequest):  # pyright: ignore[reportUnusedFunction]
            return await self.wait_for_rollouts(
                rollout_ids=request.rollout_ids, timeout=request.timeout
            )
    ...
    def _setup_metrics(self, api: APIRouter, app: FastAPI):
        ...
        def get_template_path(path: str) -> str:
            ...
            if path.endswith("/attempts/latest") and "/rollouts/" in path:
                return re.sub(
                    r"rollouts/[^/]+/attempts/latest$",
                    "rollouts/{rollout_id}/attempts/latest",
                    path,
                )
            if path.endswith("/attempts/search") and "/rollouts/" in path:
                return re.sub(
                    r"rollouts/[^/]+/attempts/search$",
                    "rollouts/{rollout_id}/attempts/search",
                    path,
                )
            ...
            path = re.sub(
                r"/rollouts/[^/]+", "/rollouts/{rollout_id}", path
            )  # Handles root and middle
        ...
        async def tracking_middleware(  # pyright: ignore[reportUnusedFunction]
            ...
            try:
                ...
                server_logger.debug(
                    f"Client disconnected (Timeout): {request.url.path}", exc_info=True
                )
    ...
    def _setup_otlp(self, api: APIRouter):
        ...
        async def _trace_handler(request: PbExportTraceServiceRequest) -> None:
            ...
            server_logger.debug(
                f"Received {len(spans)} OTLP spans: {', '.join([span.name for span in spans])}"
            )
        ...
        async def otlp_traces(request: Request):  # pyright: ignore[reportUnusedFunction]
            return await handle_otlp_export(
                request,
                PbExportTraceServiceRequest,
                PbExportTraceServiceResponse,
                _trace_handler,
                "traces",
            )
    ...
    def _setup_dashboard(self):
        ...
        if not dashboard_dir.exists():
            server_logger.error(
                "Dashboard directory not found at %s. Please build the dashboard first.",
                dashboard_dir,
            )
            return
        ...
        if not dashboard_assets_dir.exists():
            server_logger.error(
                "Dashboard assets directory not found at %s. Please build the dashboard first.",
        ...
        if not index_file.exists():
            server_logger.error(
                "Dashboard index file not found at %s. Please build the dashboard first.",
                index_file,
            )
            return
        ...
        self.app.mount(
            "/assets", CachedStaticFiles(directory=dashboard_assets_dir), name="assets"
        )
        ...
        server_logger.info(
            "Agent-lightning dashboard will be available at %s", self.endpoint
        )
    ...
    async def _call_store_method(
        ...
        if os.getpid() == self._owner_pid:
            ...
            if self.store is not None and self.store.capabilities.get(
                "thread_safe", False
            ):
                return await getattr(self.store, method_name)(*args, **kwargs)
            else:
    ...
    async def enqueue_many_rollouts(
        self, rollouts: Sequence[EnqueueRolloutRequest]
    ) -> Sequence[Rollout]:
    ...
    async def dequeue_many_rollouts(
        ...
        *,
        ...
        return await self._call_store_method(
            "dequeue_many_rollouts", limit=limit, worker_id=worker_id
        )

    async def start_attempt(
        self, rollout_id: str, worker_id: Optional[str] = None
    ) -> AttemptedRollout:
    ...
    async def update_resources(
        self, resources_id: str, resources: NamedResources
    ) -> ResourcesUpdate:
        return await self._call_store_method(
    ...
    async def get_many_span_sequence_ids(
        self, rollout_attempt_ids: Sequence[Tuple[str, str]]
    ) -> Sequence[int]:
        return await self._call_store_method(
    ...
    async def wait_for_rollouts(
        self, *, rollout_ids: List[str], timeout: Optional[float] = None
    ) -> List[Rollout]:
        return await self._call_store_method(
            "wait_for_rollouts", rollout_ids=rollout_ids, timeout=timeout
        )
...
class LightningStoreClient(LightningStore):
    ...
    def __init__(
        ...
        *,
        ...
        self._sessions: Dict[
            int, aiohttp.ClientSession
        ] = {}  # id(loop) -> ClientSession
        self._lock = threading.Lock()
    ...
    async def _wait_until_healthy(self, session: aiohttp.ClientSession) -> bool:
        ...
        if not self._health_retry_delays:
            client_logger.info(
                "No health retry delays configured; skipping health checks."
            )
            return True

        client_logger.info(
            f"Waiting for server to be healthy at {self.server_address}/health"
        )
        for delay in [*self._health_retry_delays, 0.0]:
            try:
                async with session.get(f"{self.server_address}/health") as r:
                    if r.status == 200:
                        client_logger.info(
                            f"Server is healthy at {self.server_address}/health"
                        )
                        return True
            except Exception:
                # swallow and retry
                if delay > 0.0:
                    client_logger.warning(
                        f"Server is not healthy yet. Retrying in {delay} seconds."
                    )
            if delay > 0.0:
    ...
    async def _request_json(
        ...
        *,
        ...
        for delay in attempts:
            if delay:
                client_logger.info(
                    f"Waiting {delay} seconds before retrying {method}: {path}"
                )
                await asyncio.sleep(delay)
            try:
                ...
                client_logger.debug(
                    f"ClientResponseError ({method} {path}): {cre.status} {cre.message}",
                    exc_info=True,
                )
                ...
                last_exc = cre
                client_logger.info(
                    f"5xx and other status codes will be retried. Retrying the request {method}: {path}"
                )
                ...
                client_logger.debug(
                    f"Network/session issue ({method} {path}): {net_exc}", exc_info=True
                )
                last_exc = net_exc
                client_logger.info(
                    f"Network/session issue: {net_exc} - will retry the request {method}: {path}"
                )
                if not await self._wait_until_healthy(session):
                    break  # server is not healthy, do not retry
    ...
    async def enqueue_many_rollouts(
        ...
        if not rollouts:
            return []
        request_body = EnqueueManyRolloutsRequest(rollouts=list(rollouts)).model_dump(
            exclude_none=False
        )
        data = await self._request_json(
            "post",
    ...
    async def _dequeue_batch(
        ...
        *,
        ...
        try:
            ...
            client_logger.debug(
                "dequeue_rollout failed with exception. Details:", exc_info=True
            )
    ...
    async def query_rollouts(
        ...
        *,
        ...
        resolved_rollout_ids = (
            rollout_id_in if rollout_id_in is not None else rollout_ids
        )
        ...
        return PaginatedResult(
            items=items, limit=data["limit"], offset=data["offset"], total=data["total"]
        )
    ...
    async def query_attempts(
        ...
        *,
        ...
        if sort_by is not None:
            ...
            payload["sort_order"] = sort_order
        data = await self._request_json(
            "post", f"/rollouts/{rollout_id}/attempts/search", json=payload
        )
    ...
    async def get_latest_attempt(self, rollout_id: str) -> Optional[Attempt]:
        ...
        try:
            data = await self._request_json(
                "get", f"/rollouts/{rollout_id}/attempts/latest"
            )
            return Attempt.model_validate(data) if data else None
        except Exception as e:
            client_logger.error(
                f"get_latest_attempt failed after all retries for rollout_id={rollout_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def get_rollout_by_id(self, rollout_id: str) -> Optional[Rollout]:
        ...
        try:
            ...
            client_logger.error(
                f"get_rollout_by_id failed after all retries for rollout_id={rollout_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def add_resources(self, resources: NamedResources) -> ResourcesUpdate:
        data = await self._request_json(
            "post",
            "/resources",
            json=TypeAdapter(NamedResources).dump_python(resources),
        )
    ...
    async def get_resources_by_id(self, resources_id: str) -> Optional[ResourcesUpdate]:
        ...
        try:
            ...
            client_logger.error(
                f"get_resources_by_id failed after all retries for resources_id={resources_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def get_latest_resources(self) -> Optional[ResourcesUpdate]:
        ...
        try:
            ...
            client_logger.error(
                f"get_latest_resources failed after all retries: {e}", exc_info=True
            )
            return None

    async def add_span(self, span: Span) -> Optional[Span]:
        data = await self._request_json(
            "post", "/spans", json=span.model_dump(mode="json")
        )
    ...
    async def update_worker(
        ...
        data = await self._request_json(
            "post", f"/workers/{worker_id}", json=json_payload
        )
        return Worker.model_validate(data)
