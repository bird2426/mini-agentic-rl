...
from mini_agent_lighting.algorithm import Algorithm, Baseline, FastAlgorithm
from mini_agent_lighting.client import AgentLightningClient
from mini_agent_lighting.execution.base import ExecutionStrategy
from mini_agent_lighting.execution.client_server import ClientServerExecutionStrategy
...
from mini_agent_lighting.runner import LitAgentRunner, Runner
...
from mini_agent_lighting.tracer.agentops import AgentOpsTracer
from mini_agent_lighting.tracer.base import Tracer
from mini_agent_lighting.types import Dataset, Hook, NamedResources
    ...
    * **Telemetry plumbing:** Ensures tracers, adapters, and optional [`LLMProxy`][agentlightning.LLMProxy]
    ...
    def __init__(
        ...
        *,
        ...
        self._dev = dev
        self.daemon = daemon
        self._client: AgentLightningClient | None = (
            None  # Will be initialized in fit or fit_v0
        )
    ...
    def _make_algorithm(
        self, algorithm: ComponentSpec[Algorithm]
    ) -> Optional[Algorithm]:
    ...
    def _make_adapter(
        self, adapter: ComponentSpec[TraceAdapter[Any]]
    ) -> TraceAdapter[Any]:
    ...
    def _make_store(
        self, store: ComponentSpec[LightningStore], strategy: ExecutionStrategy
    ) -> LightningStore:
    ...
    def _make_strategy(
        ...
        *,
        ...
        if isinstance(strategy, ExecutionStrategy):
            ...
            return strategy
        optional_defaults: Dict[str, Callable[[], Any]] = {
        ...
        def default_factory() -> ExecutionStrategy:
            if port is not None:
                return ClientServerExecutionStrategy(
                    n_runners=n_runners, server_port=port
                )
    ...
    def _make_runner(self, runner: ComponentSpec[Runner[Any]]) -> Runner[Any]:
        ...
        def default_runner_factory() -> Runner[Any]:
            return instantiate_component(
                LitAgentRunner, optional_defaults=optional_defaults
            )
    ...
    def _normalize_hooks(
        self, hooks: Optional[Union[Hook, Sequence[Hook]]]
    ) -> Sequence[Hook]:
    ...
    async def _runner_bundle(
        ...
        try:
            ...
            logger.exception(
                "Runner bundle encountered an error (worker_id=%s).", worker_id
            )
            raise
        finally:
            if runner_instance is not None:
                if worker_initialized:
                    try:
                        ...
                        logger.exception(
                            "Error during runner worker teardown (worker_id=%s).",
                            worker_id,
                        )
                if runner_initialized:
                    try:
                        ...
                        logger.exception(
                            "Error during runner teardown (worker_id=%s).", worker_id
                        )
