...
from mini_agent_lighting.adapter import TraceAdapter, TracerTraceToTriplet
from mini_agent_lighting.algorithm import Algorithm
from mini_agent_lighting.client import AgentLightningClient
from mini_agent_lighting.litagent import LitAgent
from mini_agent_lighting.runner import LegacyAgentRunner
from mini_agent_lighting.tracer.base import Tracer
from mini_agent_lighting.types import Dataset, ParallelWorkerBase
...
class TrainerLegacy(ParallelWorkerBase):
    ...
    def _extract_client_from_data(
        ...
        if isinstance(data, str):
            if not data.startswith("http://") and not data.startswith("https://"):
                raise ValueError(
                    "String data must be a valid URL starting with http:// or https://"
                )
    ...
    def _determine_backend(
        ...
        if self._dev:
            ...
            if client is None:
                raise ValueError(
                    "dev_data must be a string URL or AgentLightningClient when dev=True."
                )
            return client
        else:
    ...
    def client(self) -> AgentLightningClient:
        ...
        if self._client is None:
            raise RuntimeError(
                "AgentLightningClient has not been initialized. Call `init` first."
            )
        return self._client

    def _init_client(
        self, backend: Union[str, AgentLightningClient]
        ...
        if self._client is None:
            if isinstance(backend, AgentLightningClient):
                ...
                logger.info(
                    f"Initializing AgentLightningClient with endpoint: {backend}"
                )
                if not isinstance(backend, str):  # type: ignore
                    raise ValueError(
                        "backend must be a string URL or an AgentLightningClient instance."
                    )
                if not backend.startswith("http://") and not backend.startswith(
                    "https://"
                ):
                    raise ValueError(
                        "backend must be a valid URL starting with http:// or https://"
                    )
                # Initialize the client with the provided backend URL
                self._client = AgentLightningClient(endpoint=backend)
        else:
            logger.warning(
                "AgentLightningClient already initialized. Returning existing instance."
            )
        return self._client
    ...
    def _worker_main_loop(self, agent: LitAgent[Any], worker_id: int, is_async: bool):
        ...
        if not isinstance(self.triplet_exporter, TracerTraceToTriplet):  # type: ignore
            raise ValueError(
                "triplet_exporter must be a TracerTraceToTriplet for the legacy trainer."
            )
        ...
        try:
            ...
            logger.exception(
                f"[Worker {worker_id}] Unhandled exception in worker loop."
            )
        finally:
    ...
    def _initialize_worker_env(self, worker_id: int):
        logger.info(
            f"[Worker {worker_id}] Setting up trainer environment..."
        )  # worker_id included in process name
    ...
    def _terminate_processes(self, processes: List[multiprocessing.Process]) -> None:
        if self.n_workers > 1 and len(processes) > 0:
            for i, p in enumerate(processes):
                if p.is_alive():
                    logger.info(
                        f"Terminating worker {i} (name: {p.name}, PID: {p.pid})..."
                    )
                    p.terminate()
                else:
                    logger.info(
        ...
        *,
        ...
        if dev_backend is not None:
            ...
            if dev_data is not None:
                raise ValueError(
                    "dev_data and dev_backend cannot be provided at the same time."
                )
        ...
        try:
            if self.n_workers == 1:
                ...
                logger.info(
                    f"Single worker mode finished. Tasks processed: {num_tasks}"
                )
                ...
                logger.info(
                    f"Running with n_workers={self.n_workers} ({mode} multiprocessing)."
                )
                for i in range(self.n_workers):
                    ...
                    processes.append(p)
                    logger.info(
                        f"Starting worker process {i} (name: {process_name})..."
                    )
                    p.start()
                ...
                if self.daemon:
                    ...
                    if self.algorithm is not None:
                        logger.info(
                            "All workers have been spawned. Running algorithm training with provided datasets."
                        )
                        self.algorithm.run(
                            train_dataset=train_dataset,
                            val_dataset=val_dataset,
                    ...
                    logger.info(
                        f"All {self.n_workers} worker processes have completed."
                    )
                else:
                    logger.info(
                        "All worker processes started. Main process will not wait."
                    )
            ...
            if self.daemon:
                ...
                logger.info(
                    "Main process exiting. Please use Trainer.kill_orphaned_processes() for cleanup."
                )
