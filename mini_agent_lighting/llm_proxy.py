...
from mini_agent_lighting.semconv import LightningResourceAttributes
from mini_agent_lighting.types import LLM, ProxyLLM
...
def _get_pre_call_data(args: Any, kwargs: Any) -> Dict[str, Any]:
    ...
    if kwargs.get("data"):
        ...
        raise ValueError(
            f"Unable to get request data from args or kwargs: {args}, {kwargs}"
        )
...
def _reset_litellm_logging_worker() -> None:
    ...
    try:
        ...
        litellm_logging_worker.GLOBAL_LOGGING_WORKER = (
            litellm_logging_worker.LoggingWorker()
        )
        litellm_utils.GLOBAL_LOGGING_WORKER = (
            litellm_logging_worker.GLOBAL_LOGGING_WORKER
        )  # type: ignore[reportAttributeAccessIssue]
    except Exception:  # pragma: no cover - best-effort hygiene
        logger.warning(
            "Unable to propagate LiteLLM logging worker reset.", exc_info=True
        )
...
def _reset_litellm_logging_callback_manager() -> None:
    ...
    try:
        ...
        logger.warning(
            "Unable to reset LiteLLM logging callback manager.", exc_info=True
        )
...
class LightningSpanExporter(SpanExporter):
    ...
    * Export is synchronous to the caller yet schedules an async flush on the
    ...
    def __init__(self, _store: Optional[LightningStore] = None):
        self._store: Optional[LightningStore] = (
            _store  # this is only for testing purposes
        )
    ...
    def _ensure_loop(self) -> asyncio.AbstractEventLoop:
        ...
        if self._loop is None:
            ...
            self._loop_thread = threading.Thread(
                target=self._run_loop, name="LightningSpanExporterLoop", daemon=True
            )
    ...
    def _clear_loop_and_lock(self) -> None:
        ...
        if os.getpid() != self._loop_lock_pid:
            logger.warning(
                "Loop and lock are not owned by the current process. Clearing them."
            )
            self._loop = None
    ...
    def _maybe_flush(self):
        ...
        for root_span_id in self._get_root_span_ids():
            ...
            if store is None:
                logger.warning(
                    "Store is not set in LLMProxy. Cannot log spans to store."
                )
                continue
            ...
            for span in subtree_spans:
                ...
                if not headers_str.strip():
                    logger.warning(
                        "metadata.requester_custom_headers is an empty string. Skipping the span."
                    )
                    continue
                try:
            ...
            if otlp_enabled:
                ...
                if export_result != SpanExportResult.SUCCESS:
                    raise RuntimeError(
                        f"Failed to export spans via OTLP exporter. Result: {export_result}"
                    )
    ...
    * Uses [`LightningSpanExporter`][agentlightning.llm_proxy.LightningSpanExporter] to persist spans for analytics and training.
    ...
    def __init__(self):
        ...
        if _check_tracer_provider():
            logger.error(
                "Tracer is already initialized. OpenTelemetry may not work as expected."
            )
    ...
    async def async_pre_call_deployment_hook(
        ...
        """
        if (
            "metadata" not in kwargs
            or "litellm_parent_otel_span" not in kwargs["metadata"]
        ):
    ...
    async def dispatch(
        ...
        if match:
            ...
            if store is not None:
                ...
                logger.warning(
                    "Store is not set. Skipping sequence id allocation and header injection."
                )
...
class StreamConversionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        # Now the stream case
        return await self._handle_stream_case(
            request, json_body, endpoint_format, call_next
        )
    ...
    async def _handle_stream_case(
        ...
        if 200 <= response.status_code < 300:
            try:
                ...
                logger.exception(
                    f"Error converting to stream; returning non-stream response: {e}"
                )
    ...
    async def anthropic_stream_generator(self, original_response: Dict[str, Any]):
        ...
        message_delta = {
            ...
            "usage": {
                "output_tokens": original_response.get("usage", {}).get(
                    "output_tokens", 0
                )
            },
        }
    ...
    async def openai_stream_generator(
        ...
        yield sse_chunk(
            {
                ...
                "model": model,
                "choices": [
                    {"index": index, "delta": {"role": role}, "finish_reason": None}
                ],
            }
        )
        ...
        for tc_index, tc in enumerate(tool_calls):
            ...
            fn: Dict[str, Any] = (
                (tc.get("function") or {}) if tc_type == "function" else {}
            )
    ...
    * [`restart()`][agentlightning.LLMProxy.restart] convenience wrapper to stop then start.
    ...
    def __init__(
        ...
        if launcher_args is not None and (
            ...
            raise ValueError(
                "port, host, launch_mode, and num_workers cannot be set when launcher_args is provided."
            )
        ...
        if self.server_launcher_args.healthcheck_url is None:
            logger.warning(
                "healthcheck_url is not set. LLM Proxy will not be checked for healthiness after starting."
            )
        ...
        self.server_launcher = PythonServerLauncher(
            app, self.server_launcher_args, noop_context()
        )
    ...
    def initialize(self):
        ...
        * LiteLLM callbacks for token ids and OpenTelemetry export.
        ...
        if self.store is None:
            raise ValueError(
                "Store is not set. Please set the store before initializing the LLMProxy."
            )

        if _global_llm_proxy is not None:
            logger.warning(
                "A global LLMProxy is already set. Overwriting it with the new instance."
            )
        ...
        for mw in self.middlewares:
            if mw not in installation_status:
                ...
                logger.info(
                    f"Middleware {mw} is already installed. Will not install a new one."
                )
    ...
    async def _serve_context(self) -> AsyncGenerator[None, None]:
        ...
        if not self.store:
            raise ValueError(
                "Store is not set. Please set the store before starting the LLMProxy."
            )
    ...
    async def start(self):
        ...
        * Launches uvicorn in a daemon thread and waits for readiness.
        ...
        if self.server_launcher.args.launch_mode == "mp" and not store_capabilities.get(
            "zero_copy", False
        ):
            raise RuntimeError(
            ...
            self.server_launcher.args.launch_mode == "thread"
            and not store_capabilities.get("thread_safe", False)
        ):
            raise RuntimeError(
                "The store is not thread-safe. Please use another store, or use asyncio mode to launch the server."
            )
        elif (
            self.server_launcher.args.launch_mode == "asyncio"
            and not store_capabilities.get("async_safe", False)
        ):
    ...
    def as_resource(
        ...
        if model is None:
            if len(self.model_list) == 1:
                ...
                raise ValueError(
                    "No models found in model_list. Please specify the model."
                )
            else:
                first_model_name = self.model_list[0]["model_name"]
                if all(
                    model_config["model_name"] == first_model_name
        ...
        if rollout_id is None and attempt_id is None:
            ...
            raise ValueError(
                "Either rollout_id and attempt_id must be provided, or neither."
            )
...
def get_active_llm_proxy() -> LLMProxy:
    ...
    if _global_llm_proxy is None:
        raise ValueError(
            "Global LLMProxy is not set. Please call llm_proxy.start() first."
        )
    ...
    if LightningOpenTelemetry in callback_classes:
        ...
        if not _check_tracer_provider():
            ...
            _callbacks_before_litellm_start = [
                cb
                for cb in _callbacks_before_litellm_start
                if not isinstance(cb, LightningOpenTelemetry)
            ] + [LightningOpenTelemetry()]
        else:
            logger.debug(
                "Global tracer provider is valid. Reusing existing OpenTelemetry callback."
            )
