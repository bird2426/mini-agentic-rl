...
from mini_agent_lighting.types import NamedResources, Rollout, RolloutRawResult, Task

if TYPE_CHECKING:
    from mini_agent_lighting.runner import Runner
    from mini_agent_lighting.tracer import Tracer
...
class LitAgent(Generic[T]):
    ...
    def __init__(
        self, *, trained_agents: Optional[str] = None
    ) -> None:  # FIXME: str | None won't work for cli
    ...
    def is_async(self) -> bool:
        ...
        return (
            (
                ...
                and self.__class__.training_rollout_async
                is not LitAgent.training_rollout_async  # type: ignore
            )
            or (
                hasattr(self, "validation_rollout_async")
                and self.__class__.validation_rollout_async
                is not LitAgent.validation_rollout_async  # type: ignore
            )
            or (
                hasattr(self, "rollout_async")
                and self.__class__.rollout_async is not LitAgent.rollout_async
            )  # type: ignore
        )
    ...
    def get_trainer(self) -> Trainer:
        ...
        if trainer is None:
            raise ValueError(
                "Trainer reference is no longer valid (object has been garbage collected)."
            )
        return trainer
    ...
    def get_runner(self) -> Runner[T]:
        ...
        if runner is None:
            raise ValueError(
                "Runner reference is no longer valid (object has been garbage collected)."
            )
        return runner
    ...
    def on_rollout_end(
        self, task: Task, rollout: Rollout, runner: Runner[T], tracer: Tracer
    ) -> None:
    ...
    async def rollout_async(
        ...
        raise NotImplementedError(
            "Agents must implement the `rollout_async` method for async operations."
        )
