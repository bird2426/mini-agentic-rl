...
from mini_agent_lighting.types import Span
...
def group_genai_dict(
    ...
    relevant = {
        k[len(prefix) + 1 :]: v for k, v in data.items() if k.startswith(prefix + ".")
    }
    ...
    if indexed:
        ...
        for i in sorted(grouped.keys()):
            result.append(
                group_genai_dict(
                    {f"{prefix}.{rest}": val for rest, val in grouped[i].items()},
                    prefix,
                )
            )
    else:
...
def convert_to_openai_messages(
    prompt_completion_list: List[_RawSpanInfo],
            ...
            tools = [
                ChatCompletionFunctionToolParam(
                    ...
                    function={
                        ...
                        "parameters": (
                            json.loads(fn["parameters"])
                            if isinstance(fn["parameters"], str)
                            else fn["parameters"]
                        ),
                    },
                )
...
class TraceToMessages(TraceAdapter[List[OpenAIMessages]]):
    ...
    def get_tool_calls(
        self, completion: Span, all_spans: Sequence[Span], /
        ...
        for maybe_tool_call in children:
            ...
            if not isinstance(tool_call, dict):
                raise ValueError(
                    f"Extracted tool call from trace is not a dict: {tool_call}"
                )
            if tool_call:
                yield tool_call
    ...
    def adapt(self, source: Sequence[Span], /) -> List[OpenAIMessages]:
        ...
        for span in source:
            ...
            if not isinstance(completion, list):
                raise ValueError(
                    f"Extracted completion from trace is not a list: {completion}"
                )
            if not isinstance(request, dict):
                raise ValueError(
                    f"Extracted request from trace is not a dict: {request}"
                )
            if not isinstance(response, dict):
                raise ValueError(
                    f"Extracted response from trace is not a dict: {response}"
                )
