...
from mini_agent_lighting.emitter.reward import get_reward_value
from mini_agent_lighting.semconv import (
    AGL_OPERATION,
    AGL_REWARD,
    LightningSpanAttributes,
)
from mini_agent_lighting.types import Span, Triplet
from mini_agent_lighting.utils.otel import filter_and_unflatten_attributes
    ...
    for key in keys:
        if key in attributes:
            if isinstance(attributes[key], str):
                ...
                logger.warning(
                    f"Attribute {key} is found but is not a string: {attributes[key]}"
                )
    return None
...
def _attributes_get_ids_multiple(
    ...
    for key in keys:
        if key in attributes:
            if (
                isinstance(attributes[key], list) or isinstance(attributes[key], tuple)
            ) and all(isinstance(x, int) for x in attributes[key]):
                return list(attributes[key])
            else:
                logger.warning(
                    f"Attribute {key} is found but is not a list of integers: {attributes[key]}"
                )
    return None
...
class TraceTree:
    ...
    def visualize(
        self, filename: str, interested_span_match: str | None = None
    ) -> None:
    ...
    def agent_name(self) -> Optional[str]:
        ...
        if is_agent_type:
            agent_name = cast(
                Optional[str], attributes.get("agentlightning.operation.input.name")
            )
            if agent_name is not None:
                return agent_name
    ...
    def is_reward_span(self) -> bool:
        ...
        if (
            ...
            and self.span.attributes.get(LightningSpanAttributes.OPERATION_NAME.value)
            == AGL_REWARD
        ):
            return True
        ...
        *,
        ...
        if is_llm_call:
            ...
            response_id = _attributes_get_multiple(
                ...
                ["gen_ai.response.id", "agentlightning.operation.output.id"],
            )
    ...
    def match_rewards(
        self, reward_match: str, llm_calls: List["TraceTree"]
        ...
        if reward_match == RewardMatchPolicy.FIRST_OCCURRENCE:
            time_sorted: List[TraceTree] = cast(
                List[TraceTree], sorted(self.traverse(), key=lambda x: x.start_time)
            )  # type: ignore
            ...
            if isinstance(content, list):
                for content_part in cast(List[Dict[str, Any]], content):
                    ...
                    if content_part.get("type") == "image_url":
                        image_url_dict = cast(
                            Dict[str, Any], content_part.get("image_url")
                        )
    ...
    def span_to_triplet(self, span: Span, agent_name: str) -> Triplet:
        ...
        completion_raw_content = _attributes_unflatten_multiple(
            span.attributes,
            ["gen_ai.completion", "agentlightning.operation.output.choices"],
        )
        image_urls = self.extract_prompt_image_urls(prompt_raw_content)
        prompt_payload = {
            "token_ids": prompt_token_ids,
            "raw_content": prompt_raw_content,
            "image_urls": image_urls,
        }
        response_payload = {
            "token_ids": response_token_ids,
            "raw_content": completion_raw_content,
        }
    ...
    def to_trajectory(
        ...
        for llm_call, agent_name in llm_calls:
            ...
            if _skip_empty_token_spans and (
                not triplet.prompt.get("token_ids")
                or not triplet.response.get("token_ids")
            ):
                logger.warning(
                    f"Skipping LLM call with unrecorded token IDs: {triplet}"
                )
                continue
            filtered_llm_calls.append((llm_call, agent_name))
            id_transitions.append((llm_call.id, triplet))

        rewards = self.match_rewards(
            reward_match, [call for call, _ in filtered_llm_calls]
        )
        transitions = [
        ...
        if final_reward is not None and len(transitions) > 0:
            ...
            transitions[-1] = transitions[-1].model_copy(
...
class TracerTraceToTriplet(TraceToTripletBase):
    ...
    def visualize(
        ...
        """
        source_normalized = [
            Span.from_opentelemetry(span, "dummy", "dummy", 0)
    ...
    def adapt(
        self, source: Union[Sequence[Span], Sequence[ReadableSpan]], /
    ) -> List[Triplet]:  # type: ignore
...
class LlmProxyTraceToTriplet(TraceToTripletBase):
    ...
    def _extract_tokens_from_openai(
        self, attrs: Dict[str, Any]
    ) -> Tuple[List[int], List[int]]:
        ...
        if not (
            isinstance(prompt_ids, list) and all(isinstance(x, int) for x in prompt_ids)
        ):  # type: ignore
            prompt_ids = []
        if not (
            isinstance(resp_ids, list) and all(isinstance(x, int) for x in resp_ids)
        ):  # type: ignore
            resp_ids = []
