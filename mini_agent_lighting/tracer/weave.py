...
from mini_agent_lighting.semconv import (
...
from mini_agent_lighting.utils.otel import (
...
class WeaveSpanRecordingContext(SpanRecordingContext):
    ...
    def _get_output_from_attributes(self, attributes: Attributes) -> Any:
        if LightningSpanAttributes.OPERATION_OUTPUT.value in attributes:
            ...
            return filter_and_unflatten_attributes(
                attributes, LightningSpanAttributes.OPERATION_OUTPUT.value
            )
    ...
    def record_attributes(self, attributes: Attributes) -> None:
        ...
        if output_attributes:
            if self._call.output is not None:
                logger.warning(
                    f"Output is already set. It will be overridden: {self._call.output}"
                )
        ...
        for key, value in attributes.items():
            if (
                ...
                and not key.startswith(
                    LightningSpanAttributes.OPERATION_INPUT.value + "."
                )
    ...
    def get_recorded_span(self) -> SpanCoreFields:
        return SpanCoreFields(
            ...
            start_time=self._call.started_at.timestamp()
            if self._call.started_at
            else None,
            end_time=self._call.ended_at.timestamp() if self._call.ended_at else None,
            status=TraceStatus(
                status_code="OK" if self._call.exception is None else "ERROR",
                description=self._call.exception,
            ),
        )
        ...
        with self._call_threading_lock:
            if call_id in self.calls:
                if call_id not in self._calls_already_invoked:
                    ...
                    logger.info(
                        f"Call {call_id} has callback already invoked. Skipping."
                    )
...
class WeaveTracer(Tracer):
    ...
    def __init__(
        ...
        *,
        ...
        self.weave_user_settings = weave_user_settings or UserSettings(
            use_server_cache=False
        )

        self._store: Optional[LightningStore] = None
        self._server = WeaveTracerManagedTraceServer(
            partial_call_callback=self.partial_call_callback,
            complete_call_callback=self.complete_call_callback,
        )
        ...
        self._partial_call_futures: Dict[
            str, asyncio.Future[int] | futures.Future[int]
        ] = {}
        self._complete_call_futures: List[
            asyncio.Future[None] | futures.Future[None]
        ] = []
    ...
    def init_worker(self, worker_id: int, store: Optional[LightningStore] = None):
        ...
        if not os.getenv("WANDB_API_KEY"):
            ...
            logger.debug(
                "WANDB_API_KEY is set. Weave will be initialized automatically."
            )
        ...
        if weave_client is not None:
            logger.warning(
                "Weave client was already initialized. Reentrant calls are at your own risk."
            )
        ...
        try:
            weave.init(
                project_name=self.project_name, settings=self.weave_user_settings
            )
            logger.info(f"[Worker {worker_id}] Weave client initialized.")
        except Exception as exc:
            raise RuntimeError(
                f"Failed to initialize Weave for project '{self.project_name}'"
            ) from exc
    ...
    async def trace_context(
        ...
        **kwargs: Any,
        ...
        if rollout_id is not None and attempt_id is not None:
            ...
            logger.info(
                "No rollout_id or attempt_id provided. Skipping writing to store."
            )
            self._rollout_id = self._attempt_id = None
        else:
            raise ValueError(
                "rollout_id and attempt_id must be either both provided or both None"
            )
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Trace failed for rollout_id={rollout_id}, attempt_id={attempt_id}, error={exc}"
                )
                raise
    ...
    def create_span(
        ...
        if timestamp is not None:
            logger.warning(
                "Weave doesn't support customizing the start time of a call. Timestamp is ignored."
            )
        weave_client = self._get_weave_client()
        trace_call = weave_client.create_call(  # pyright: ignore[reportUnknownMemberType]
            op=name,
        ...
        start_time = (
            trace_call.started_at.timestamp() if trace_call.started_at else None
        )
    ...
    def operation_context(
        ...
        if end_time is not None:
            logger.warning(
                "Weave doesn't support customizing the end time of a call. Timestamp is ignored."
            )
        weave_client = self._get_weave_client()
        trace_call = weave_client.create_call(  # pyright: ignore[reportUnknownMemberType]
            op=name,
    ...
    def _get_weave_client(self) -> WeaveClient:
        ...
        if not weave_client:
            raise RuntimeError(
                "Weave client is not initialized. Call init_worker() first."
            )
        return weave_client
        ...
        try:
            ...
            if is_current_loop:
                ...
                task = asyncio.run_coroutine_threadsafe(
                    self.partial_call_handler(request_content), loop
                )
    ...
    def complete_call_callback(self, call: tsi.CallSchema) -> None:
        try:
            ...
            if is_current_loop:
                ...
                # Schedule the task on the dedicated loop
                task = asyncio.run_coroutine_threadsafe(
                    self.complete_call_handler(call), loop
                )
    ...
    async def _get_next_sequence_id(self) -> int:
        ...
        if self._rollout_id and self._attempt_id and self._store:
            return await self._store.get_next_span_sequence_id(
                self._rollout_id, self._attempt_id
            )
        else:
    ...
    async def complete_call_handler(self, call: tsi.CallSchema) -> None:
        ...
        if call.id in self._partial_call_futures:
            ...
            if call.id in self._calls:
                ...
                logger.warning(
                    f"Call {call.id} has no start future. Fetching a new sequence ID."
                )
            sequence_id = await self._get_next_sequence_id()

        self._calls[call.id] = call

        span = await self.convert_call_to_span(
            call, self._rollout_id, self._attempt_id, sequence_id
        )
    ...
    async def convert_call_to_span(
        ...
        sanitized_attributes = sanitize_attributes(
            flatten_attributes(attributes, expand_leaf_lists=False)
        )
