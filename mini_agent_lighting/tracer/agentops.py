...
from mini_agent_lighting.utils.otel import get_span_processors, get_tracer_provider
...
class AgentOpsTracer(OtelTracer):
    ...
    def __init__(
        ...
        *,
        ...
        if not self.agentops_managed:
            logger.warning(
                "agentops_managed=False. You are responsible for AgentOps setup."
            )
        if not self.instrument_managed:
            logger.warning(
                "instrument_managed=False. You are responsible for all instrumentation."
            )
    ...
    def _initialize_tracer_provider(self, worker_id: int):
        logger.info(
            f"[Worker {worker_id}] Setting up AgentOps tracer..."
        ...
        if self.agentops_managed:
            ...
            if not agentops.get_client().initialized:
                ...
                logger.warning(
                    f"[Worker {worker_id}] AgentOps client was already initialized. Skip initialization."
                )

        span_processors = get_span_processors(
            self._get_tracer_provider(), LightningSpanProcessor
        )
        if len(span_processors) > 0:
            ...
            if len(span_processors) > 1:
                logger.error(
                    "More than one LightningSpanProcessors present in TracerProvider. This should not happen."
                )
            self._lightning_span_processor = span_processors[0]
        else:
            self._lightning_span_processor = LightningSpanProcessor()
            self._get_tracer_provider().add_span_processor(
                self._lightning_span_processor
            )  # type: ignore
    ...
    def _agentops_trace_context(
        ...
        try:
            ...
            logger.error(
                f"Trace failed for rollout_id={rollout_id}, attempt_id={attempt_id}: {e}"
            )
    ...
    def _get_tracer_provider(self) -> TracerProviderImpl:
        try:
            ...
            if not isinstance(instance.provider, TracerProviderImpl):  # type: ignore
                raise RuntimeError(
                    "Unsupported TracerProvider type for AgentOps instrumentation."
                )
