...
from mini_agent_lighting.types.tracer import convert_timestamp
from mini_agent_lighting.utils.otel import get_tracer_provider
...
class OtelTracer(Tracer):
    ...
    def _initialize_tracer_provider(self, worker_id: int):
        ...
        if self._initialized:
            logger.info(
                f"[Worker {worker_id}] Tracer provider is already initialized. Skipping initialization."
            )
            return
        ...
        try:
            ...
            logger.debug(
                f"[Worker {worker_id}] Tracer provider is not initialized by OtelTracer. Initializing it now."
            )
    ...
    def teardown_worker(self, worker_id: int):
        ...
        logger.info(
            f"[Worker {worker_id}] Tearing down OpenTelemetry tracer does NOT remove the tracer provider."
        )
    ...
    async def trace_context(
        ...
        *,
        ...
        if rollout_id is not None and attempt_id is not None:
            if store is None:
                raise ValueError(
                    "store is required to be initialized when rollout_id and attempt_id are provided"
                )
            if store.capabilities.get("otlp_traces", False) is True:
                logger.debug(
                    f"Tracing to LightningStore rollout_id={rollout_id}, attempt_id={attempt_id}"
                )
            ...
            with ctx:
                yield trace_api.get_tracer(
                    __name__, tracer_provider=self._tracer_provider
                )
            ...
            raise ValueError(
                "rollout_id and attempt_id must be either all provided or all None"
            )
    ...
    def create_span(
        ...
        span = tracer.start_span(
            ...
            start_time=int(timestamp * 1_000_000_000) if timestamp else None,
        )
    ...
    def operation_context(
        ...
        if end_time is not None:
            logger.warning(
                "OpenTelemetry doesn't support customizing the end time of a span. End time is ignored."
            )
        ...
        with tracer.start_as_current_span(
            name,
            attributes=attributes,
            start_time=int(start_time * 1_000_000_000) if start_time else None,
        ) as span:
    ...
    def _get_tracer_provider(self) -> TracerProviderImpl:
        if self._tracer_provider is None:
            raise RuntimeError(
                "TracerProvider is not initialized. Call init_worker() first."
            )
        if not isinstance(self._tracer_provider, TracerProviderImpl):
            raise TypeError(
                f"TracerProvider is not a opentelemetry.sdk.trace.TracerProvider: {self._tracer_provider}"
            )
        return self._tracer_provider

    def _enable_native_otlp_exporter(
        ...
        for processor in active_span_processor._span_processors:  # pyright: ignore[reportPrivateUsage]
            if isinstance(processor, LightningSpanProcessor):
                ...
                logger.debug(
                    "LightningSpanProcessor already present in TracerProvider, disabling it."
                )
                ...
                if isinstance(processor.span_exporter, LightningStoreOTLPExporter):
                    ...
                    logger.debug(
                        f"Set LightningStoreOTLPExporter endpoint to {store.otlp_traces_endpoint()}"
                    )
                    instrumented = True
                else:
                    candidates.append(
...
class LightningSpanProcessor(SpanProcessor):
    ...
    def _ensure_loop(self) -> None:
        ...
        with self._loop_init_lock:
            ...
            self._loop_thread = threading.Thread(
                target=self._loop_runner, name="otel-loop", daemon=True
            )
    ...
    def with_context(self, store: LightningStore, rollout_id: str, attempt_id: str):
        ...
        class _Ctx:
            def __enter__(_):  # type: ignore
                ...
                with self._lock:
                    self._store, self._rollout_id, self._attempt_id = (
                        store,
                        rollout_id,
                        attempt_id,
                    )
    ...
    def on_end(self, span: ReadableSpan) -> None:
        ...
        if (
            not self._disable_store_submission
            and self._store
            and self._rollout_id
            ...
            try:
                ...
                with suppress_instrumentation():
                    ...
                    uploaded_span = self._await_in_loop(
                        self._store.add_otel_span(
                            self._rollout_id, self._attempt_id, span
                        ),
                ...
                logger.exception(
                    f"Error adding span to store: {span.name}. The span will be store locally only."
                )
                self._spans.append(
                    Span.from_opentelemetry(
                        span,
